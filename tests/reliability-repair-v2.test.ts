...
test('authenticated app preserves its route and exposes retry instead of silently falling back on service errors', () => {
  assert.equal(app.includes("type GateState = 'loading' | 'legacy' | 'unavailable' | 'ready'"), true);
  assert.equal(app.includes('class RequestError extends Error'), true);
  assert.equal(app.includes('function isAuthenticationBoundary'), true);
  assert.equal(app.includes("error.status === 401 || error.status === 403"), true);
  ...
  assert.match(app, /catch \(error\) \{[\s\S]*setState\(isAuthenticationBoundary\(error\) \? 'legacy' : 'unavailable'\)/);
});

test('Team invitation gate distinguishes service failure from signed-out state', () => {
  ...
  assert.equal(gate.includes("setState(isAuthenticationBoundary(error) ? 'legacy' : 'unavailable')"), true);
