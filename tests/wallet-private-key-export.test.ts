...
test('Linkary wallet export uses the CDP isolated export UI with 2-step verification and explicit consent', () => {
  assert.equal(walletSource.includes("ExportWalletModal, EnrollMfaModal, VerifyMfaModal, SignInModal"), true);
...
test('expired or missing Coinbase secure sessions have an explicit recovery action', () => {
  assert.equal(walletSource.includes('const [secureSessionExpired,setSecureSessionExpired]=useState(false);'), true);
  assert.equal(walletSource.includes('const secureSessionUnavailable=!currentUser||secureSessionExpired;'), true);
  assert.equal(walletSource.includes('>Restore secure wallet session</button>'), true, 'wallet export must not dead-end when the Coinbase session is unavailable');
  assert.equal(walletSource.includes('<SignInModal onSuccess='), true, 'session recovery must use Coinbase CDP sign-in UI rather than a Linkary credential flow');
  assert.equal(walletSource.includes("setMessage('Secure Coinbase wallet session restored. Continue with 2-step verification before exporting.')"), true);
  assert.equal(walletSource.includes("setMessage('Secure Coinbase wallet session expired. Restore it before trying private key export again.')"), true);
  assert.equal(walletSource.includes('setSecureSessionExpired(true);setExportOpen(false);setExportAcknowledged(false);setExportVerified(false);'), true, 'an expired secure session must close and reset the active export attempt');
});
...
test('every private-key export attempt requires a fresh 2-step verification challenge', () => {
  ...
  assert.equal(walletSource.includes('setExportVerified(false);setSecureSessionExpired(false);setSecurityOpen(false);'), true, 'successful export must reset the verification and secure-session gates');
...
test('advanced wallet security is closed by default and must be deliberately opened', () => {
  ...
  assert.equal(walletSource.includes('setSecurityOpen(false);setExportOpen(false);setExportAcknowledged(false);setExportVerified(false);setSecureSessionExpired(false);'), true);
});
