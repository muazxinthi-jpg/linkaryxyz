...
test('accepted Network Map remains the fluid avatar experience with safe controls', () => {
  ...
  assert.equal(interactiveMap.includes('node.avatarUrl'), true);
  assert.equal(interactiveMap.includes('onPointerDown'), true);
