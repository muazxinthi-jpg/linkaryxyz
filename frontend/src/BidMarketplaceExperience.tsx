...
function ProfileList({ items, page, pageSize, empty }: { items: ProfileRow[]; page: number; pageSize: number; empty: string }) {
  ...
  return <div className="bid-market-auctions bid-market-card-grid">{items.map((item, index) => {
    ...
    const isPaidLiveBanner = Boolean(item.banner_ends_at);
...
export default function BidMarketplaceExperience({ me, status }: { me: ProductMe; status: ProductStatus }) {
  ...
  return <ProductWorkspace me={me} status={status} profile={profile as ProductProfile} onProfileChange={changeProfile}><div className="ops-stack bid-marketplace">
    ...
    <div className="bid-market-toolbar"><div className="bid-market-tabs bid-market-primary-tabs" role="tablist" aria-label="Bid marketplace leaderboards">{tabs.map(([value, label]) => <button key={value} role="tab" aria-selected={tab === value} type="button" className={tab === value ? 'active' : ''} onClick={() => switchTab(value)}>{label}</button>)}</div><div className="bid-market-tabs bid-market-periods" role="group" aria-label="Profile view ranking period">{([['24h', '24H'], ['7d', '7D'], ['30d', '30D'], ['all', 'All Time']] as Array<[Period, string]>).map(([value, label]) => <button key={value} type="button" aria-pressed={period === value} className={period === value ? 'active' : ''} onClick={() => switchPeriod(value)}>{label}</button>)}</div></div>
    ...
    {data && (tab === 'active' || tab === 'views' || tab === 'bids') && <>{tab === 'active' && <div className="bid-market-expiry-note"><strong>Expiry leaderboard</strong><span>Creators open for bidding are ordered by the nearest auction closing time.</span></div>}<ProfileList items={profileList?.items || []} page={page} pageSize={data.pageSize} empty={tab === 'views' ? 'No eligible public profiles yet.' : tab === 'bids' ? 'No profiles have received bids yet.' : 'No creators are open for bidding right now.'} /><Pagination page={page} total={profileList?.total || 0} pageSize={data.pageSize} onPage={setPage} /></>}
