...
function ProfileList({ items, page, pageSize, empty }: { items: ProfileRow[]; page: number; pageSize: number; empty: string }) {
  ...
  return <div className="bid-market-auctions bid-market-card-grid">{items.map((item, index) => {
    ...
    const endsAt = item.expires_at;
    return <article className="bid-market-card bid-market-visual-card" key={item.profile_id}>
      <div className="bid-card-cover">{cover ? <img src={cover} alt="" onError={(event) => { event.currentTarget.style.display = 'none'; }} /> : <div className="bid-card-cover-fallback">{item.display_name.slice(0, 1).toUpperCase()}</div>}<span className={item.auction_id ? 'bid-card-status live' : 'bid-card-status'}>{item.auction_id ? 'OPEN FOR BIDDING' : 'PROFILE'}</span><span className="bid-card-rank">{rank(index, page, pageSize)}</span>{item.auction_id && endsAt && <span className="bid-card-countdown">Auction ends {timeLeft(endsAt)}</span>}</div>
      <div className="bid-card-body"><div className="bid-card-identity"><div className="bid-card-avatar">{item.avatar_url ? <img src={item.avatar_url} alt="" /> : item.display_name.slice(0, 1).toUpperCase()}</div><div><strong>{item.display_name}</strong><span>@{item.username}</span></div><em>{item.profile_type}</em></div><div className="bid-card-tags"><span>{item.views.toLocaleString()} views</span><span>{item.bid_count.toLocaleString()} bids</span></div><div className="bid-card-price"><small>{item.auction_id ? 'Current bid' : 'Auction'}</small><strong>{item.auction_id ? money(current) : 'Banner not available'}</strong></div><div className="bid-card-availability">{item.auction_id ? `Auction closes ${new Date(item.expires_at || '').toLocaleString()}` : 'Available now'}</div><div className="bid-market-actions"><a className="ops-button secondary" href={`https://linkary.xyz/${item.username}`} target="_blank" rel="noreferrer">View profile ↗</a>{item.auction_id ? <NavLink className="ops-button primary" to={`/promotion-auction/${item.auction_id}`}>Place bid</NavLink> : null}</div></div>
    </article>;
  })}</div>;
}
