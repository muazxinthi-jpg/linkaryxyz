...
function ProfileList({ items, page, pageSize, empty }: { items: ProfileRow[]; page: number; pageSize: number; empty: string }) {
  ...
  return <div className="bid-market-auctions bid-market-card-grid">{items.map((item, index) => {
    ...
    const isLiveBanner = Boolean(item.banner_url);
    return <article className="bid-market-card bid-market-visual-card" key={item.profile_id}>
      <div className="bid-card-cover">{cover ? <img src={cover} alt="" onError={(event) => { event.currentTarget.style.display = 'none'; }} /> : <div className="bid-card-cover-fallback">{item.display_name.slice(0, 1).toUpperCase()}</div>}<span className={isLiveBanner || item.auction_id ? 'bid-card-status live' : 'bid-card-status'}>{isLiveBanner || item.auction_id ? 'LIVE' : 'PROFILE'}</span><span className="bid-card-rank">{rank(index, page, pageSize)}</span>{endsAt && <span className="bid-card-countdown">{isLiveBanner ? 'Banner ends' : 'Auction ends'} {timeLeft(endsAt)}</span>}</div>
      <div className="bid-card-body"><div className="bid-card-identity"><div className="bid-card-avatar">{item.avatar_url ? <img src={item.avatar_url} alt="" /> : item.display_name.slice(0, 1).toUpperCase()}</div><div><strong>{item.display_name}</strong><span>@{item.username}</span></div><em>{item.profile_type}</em></div><div className="bid-card-tags"><span>{item.views.toLocaleString()} views</span><span>{item.bid_count.toLocaleString()} bids</span></div><div className="bid-card-price"><small>{item.auction_id ? 'Current bid' : isLiveBanner ? 'Live banner' : 'Auction'}</small><strong>{item.auction_id ? money(current) : isLiveBanner ? 'Live now' : 'Banner not available'}</strong></div><div className="bid-card-availability">{isLiveBanner && item.banner_ends_at ? `Banner live until ${new Date(item.banner_ends_at).toLocaleString()}` : item.auction_id ? `Auction closes ${new Date(item.expires_at || '').toLocaleString()}` : 'Available now'}</div><div className="bid-market-actions"><a className="ops-button secondary" href={`https://linkary.xyz/${item.username}`} target="_blank" rel="noreferrer">View profile ↗</a>{item.auction_id ? <NavLink className="ops-button primary" to={`/promotion-auction/${item.auction_id}`}>Place bid</NavLink> : null}</div></div>
    </article>;
  })}</div>;
}
...
export default function BidMarketplaceExperience({ me, status }: { me: ProductMe; status: ProductStatus }) {
  ...
  return <ProductWorkspace me={me} status={status} profile={profile as ProductProfile} onProfileChange={changeProfile}><div className="ops-stack bid-marketplace">
    ...
    {data && (tab === 'active' || tab === 'views' || tab === 'bids') && <>{tab === 'active' && <div className="bid-market-expiry-note"><strong>Expiry leaderboard</strong><span>Live banners and open auctions are ordered by the nearest end time.</span></div>}<ProfileList items={profileList?.items || []} page={page} pageSize={data.pageSize} empty={tab === 'views' ? 'No eligible public profiles yet.' : tab === 'bids' ? 'No profiles have received bids yet.' : 'No live banners or auctions right now.'} /><Pagination page={page} total={profileList?.total || 0} pageSize={data.pageSize} onPage={setPage} /></>}
