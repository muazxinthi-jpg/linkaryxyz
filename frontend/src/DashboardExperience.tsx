...
export default function DashboardExperience({me,status}:{me:ProductMe;status:ProductStatus}){
  ...
  return <ProductWorkspace me={me} status={status} profile={profile as ProductProfile} onProfileChange={changeProfile}>
    <div className="ops-stack dashboard-next">
      <div className="dashboard-next-hero"><div><span className="ops-kicker">OVERVIEW</span><h1>{projectMode?profile.display_name:`Welcome, ${status.user.displayName||profile.display_name}.`}</h1><p>{projectMode?'See tracked growth, distribution partners and attributable outcomes across this Project.':'Your identity, partner access and attributable work live in one place.'}</p></div><a className="ops-button secondary" href={`https://linkary.xyz/${profile.username}`} target="_blank" rel="noreferrer">Public profile ↗</a></div>
      {projectMode&&project&&project.verification_status!=='verified_x'&&<section className="ops-callout verification"><div><span className="ops-kicker">ACTION REQUIRED</span><h3>Verify {project.name} with its official X account</h3><p>Project growth tracking stays locked until the Project identity is verified.</p></div><NavLink className="ops-button secondary" to="/settings">Open Projects</NavLink></section>}
      <section className="dashboard-next-metrics">
        {projectMode?<><article><span>TRACKED CAMPAIGNS</span><strong>{campaigns.length}</strong><small>Run anywhere</small></article><article><span>TRACKED CLICKS</span><strong>{totals.tracked_clicks.toLocaleString()}</strong><small>Across growth activity</small></article><article><span>OUTCOMES</span><strong>{totals.conversions.toLocaleString()}</strong><small>{(totals.conversion_rate*100).toFixed(totals.conversion_rate?1:0)}% conversion</small></article><article><span>ATTRIBUTED VALUE</span><strong>{money(totals.value_usd)}</strong><small>Recorded outcomes</small></article></>:<><article><span>PROFILE</span><strong>{profile.visibility==='published'?'Published':'Draft'}</strong><small>{linkClicks===null?'Public identity':`${linkClicks} link click${linkClicks===1?'':'s'}`}</small></article><article><span>INVITES</span><strong>{balance?.available_credits??'N/A'}</strong><small>{balance?`${balance.lifetime_used} used`:'Network access'}</small></article><article><span>WALLETS</span><strong>{walletCount}</strong><small>Additional destinations</small></article><article><span>PROJECT RELATIONSHIPS</span><strong>{networkCount}</strong><small>{project?'Across a managed Project':'Connect to a Project'}</small></article></>}
      </section>
      <section className="dashboard-next-actions"><NavLink to="/campaigns"><span>01</span><div><strong>Growth</strong><small>Track campaigns wherever they are already running.</small></div><b>→</b></NavLink><NavLink to="/partners"><span>02</span><div><strong>Partners</strong><small>Find Community Managers, KOL Managers and the audiences they represent.</small></div><b>→</b></NavLink><NavLink to="/tracking"><span>03</span><div><strong>Evidence</strong><small>Inspect tracking links, outcomes and attributable value.</small></div><b>→</b></NavLink><NavLink to="/wallets"><span>04</span><div><strong>Wallets</strong><small>Set EVM and Solana reward destinations.</small></div><b>→</b></NavLink></section>
    </div>
  </ProductWorkspace>;
}
