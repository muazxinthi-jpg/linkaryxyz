...
export default function PartnerDirectoryExperience({ me, status }: { me: ProductMe; status: ProductStatus }) {
  ...
  async function openManager(manager: Manager) {
    setSelected(manager); setAssets([]);
    try { const result = await api<{ assets: Asset[] }>(`/api/partner-manager-assets?managerId=${encodeURIComponent(manager.id)}`); setAssets(result.assets); }
  ...
  return <ProductWorkspace me={me} status={status} profile={profile as ProductProfile} onProfileChange={changeProfile}>
    <div className="ops-stack partner-directory">
      <div className="ops-heading-row">
        <div><span className="ops-kicker">PARTNER INTELLIGENCE</span><h1>Find the people behind distribution</h1><p>Discover Community Managers and KOL Managers, see the audiences they represent, and contact the right POC before you spend.</p></div>
      ...
      <section className="partner-summary-strip">
        ...
        <div><strong>Evidence first</strong><span>Overlap is only shown when an estimate exists. We never invent unique audience.</span></div>
      </section>
      ...
      <section className="ops-section">
        ...
        {loading ? <div className="ops-loading">Loading partner directory...</div> : !managers.length ? <div className="ops-empty"><div className="ops-empty-icon">◎</div><h3>No listings yet</h3><p>Early Linkary users can be the first verified points of contact in this directory.</p>{personalProfile && <button className="ops-button secondary" onClick={openListingEditor}>Create my listing</button>}</div> : <div className="partner-grid">{managers.map((manager) => <article className="partner-card" key={manager.id} onClick={() => void openManager(manager)}>
          ...
          <div className="partner-card-foot"><span>{manager.open_to_campaigns ? 'Open to opportunities' : 'Directory listing'}</span><button type="button">View portfolio →</button></div>
        </article>)}</div>}
      </section>
      ...
      {showListing && <div className="ops-modal-backdrop" onMouseDown={(event) => { if (event.currentTarget === event.target) setShowListing(false); }}><form className="ops-modal" onSubmit={saveListing}>
        ...
        <label>Estimated unique audience, optional<input type="number" min="0" value={listingForm.estimatedUniqueAudience} onChange={(e) => setListingForm({ ...listingForm, estimatedUniqueAudience: e.target.value })} placeholder="Only if you have a defensible estimate" /><small>Linkary calculates raw combined audience from your portfolio. Enter unique audience only when you have evidence or a reasonable methodology.</small></label>
        <label>Audience methodology, optional<textarea value={listingForm.audienceMethodology} onChange={(e) => setListingForm({ ...listingForm, audienceMethodology: e.target.value })} placeholder="How did you estimate overlap / unique reach?" /></label>
