...
function OnboardingScreen({ status, onComplete }: { status: OnboardingStatus; onComplete: (status: OnboardingStatus) => void }) {
  ...
  useEffect(() => { if (accountType === 'project' && status.xIdentity?.current_handle) setUsername(status.xIdentity.current_handle.toLowerCase()); }, [accountType, status.xIdentity?.current_handle]);
      ...
      <section className="onboarding-card">
        ...
        <form onSubmit={submit}>
          <div className="role-grid-real">
            ...
            {allowed.includes('project') && <button type="button" className={accountType === 'project' ? 'selected' : ''} onClick={() => setAccountType('project')}><span>▦</span><strong>Register a Project</strong><small>Claim the Project using its official X account, then run campaigns and attribution.</small></button>}
          </div>
          {status.xIdentity?.current_handle && <div className="identity-note"><span>𝕏</span><div><strong>{accountType === 'project' ? 'This X account will claim the Project' : 'X identity connected'}</strong><small>@{status.xIdentity.current_handle}. {accountType === 'project' ? 'The Project username is locked to this verified handle.' : 'Your history remains connected even if the handle changes.'}</small></div></div>}
          <div className="field-grid">
            <label>Linkary username<div className="username-field"><span>linkary.xyz/</span><input value={username} readOnly={accountType === 'project' || (accountType === 'creator' && !status.xIdentity?.current_handle)} onChange={(event) => setUsername(event.target.value.toLowerCase().replace(/[^a-z0-9_]/g, ''))} placeholder={accountType === 'creator' && !status.xIdentity?.current_handle ? 'Connect X to claim a username' : 'username'} minLength={3} maxLength={30} required={Boolean(status.xIdentity?.current_handle) || accountType === 'project'} /></div><small>{accountType === 'project' ? 'Locked to the verified Project X handle.' : status.xIdentity?.current_handle ? 'Your X identity is connected. Choose your public username.' : 'Connect X to claim a public username. Until then, Linkary uses your sign-in identity privately.'}</small></label>
          ...
          {error && <div className="form-error">{error}</div>}
          <button className="button primary full large" disabled={busy || (accountType === 'project' && !status.xIdentity?.current_handle)}>{busy ? 'Creating workspace...' : accountType === 'project' ? 'Register verified Project' : 'Create my Linkary workspace'}</button>
...
function ProjectAccessPage({ profile }: { profile: ProfileSummary }) {
  type Project = { organization_id: string; name: string; username: string };
  type RequestItem = { id: string; requested_role: string; note: string; display_name: string; email: string | null };
  type Member = { user_id: string; role: 'owner' | 'admin' | 'marketing_manager' | 'analyst' | 'viewer'; billing_manager: number; display_name: string; email: string | null };
  const [query, setQuery] = useState('');
  const [projects, setProjects] = useState<Project[]>([]);
  const [role, setRole] = useState('marketing_manager');
  const [note, setNote] = useState('');
  const [message, setMessage] = useState('');
  const [myRequests, setMyRequests] = useState<Array<{ id: string; name: string; username: string; requested_role: string; status: string }>>([]);
  const [requests, setRequests] = useState<RequestItem[]>([]);
  const [members, setMembers] = useState<Member[]>([]);
  const projectId = profile.organization_id;
  const isProject = profile.profile_type === 'project' && Boolean(projectId);
  const search = async () => { try { const r = await apiJson<{ projects: Project[] }>(`/api/projects/search?query=${encodeURIComponent(query)}`); setProjects(r.projects); } catch (err) { setMessage(publicError(err, 'Unable to search registered Projects.')); } };
  const loadMyRequests = async () => { try { const r = await apiJson<{ requests: typeof myRequests }>('/api/projects/access-requests/mine'); setMyRequests(r.requests); } catch { setMyRequests([]); } };
  const request = async (organizationId: string) => { const csrf = readCookie('__Host-linkary_csrf'); if (!csrf) return; try { await apiJson(`/api/projects/${organizationId}/access-requests`, { method: 'POST', headers: { 'x-csrf-token': csrf }, body: JSON.stringify({ role, note }) }); setNote(''); setMessage('Access request sent to the Project Admins.'); await loadMyRequests(); } catch (err) { setMessage(publicError(err, 'Unable to send access request.')); } };
  const loadAdminData = async () => { if (!projectId) return; try { const [requestResult, memberResult] = await Promise.all([apiJson<{ requests: RequestItem[] }>(`/api/projects/${projectId}/access-requests`), apiJson<{ members: Member[] }>(`/api/projects/${projectId}/members`)]); setRequests(requestResult.requests); setMembers(memberResult.members); } catch { setRequests([]); setMembers([]); } };
  useEffect(() => { void loadAdminData(); }, [projectId]);
  useEffect(() => { void loadMyRequests(); }, []);
  const cancelRequest = async (id: string) => { const csrf = readCookie('__Host-linkary_csrf'); if (!csrf) return; try { await apiJson(`/api/projects/access-requests/${id}/cancel`, { method: 'POST', headers: { 'x-csrf-token': csrf } }); await loadMyRequests(); } catch (err) { setMessage(publicError(err, 'Unable to cancel access request.')); } };
  const review = async (id: string, decision: 'approve' | 'reject') => { const csrf = readCookie('__Host-linkary_csrf'); if (!csrf) return; try { await apiJson(`/api/projects/access-requests/${id}/${decision}`, { method: 'POST', headers: { 'x-csrf-token': csrf } }); await loadAdminData(); } catch (err) { setMessage(publicError(err, 'Unable to review access request.')); } };
  const updateMember = async (userId: string, nextRole: string) => { if (!projectId) return; const csrf = readCookie('__Host-linkary_csrf'); if (!csrf) return; try { await apiJson(`/api/projects/${projectId}/members/${userId}`, { method: 'PATCH', headers: { 'x-csrf-token': csrf }, body: JSON.stringify({ role: nextRole }) }); setMessage('Project member role updated.'); await loadAdminData(); } catch (err) { setMessage(publicError(err, 'Unable to update member role.')); } };
  const removeMember = async (userId: string) => { if (!projectId || !window.confirm('Remove this person from the Project? They will immediately lose Project access.')) return; const csrf = readCookie('__Host-linkary_csrf'); if (!csrf) return; try { await apiJson(`/api/projects/${projectId}/members/${userId}`, { method: 'DELETE', headers: { 'x-csrf-token': csrf } }); setMessage('Project member removed.'); await loadAdminData(); } catch (err) { setMessage(publicError(err, 'Unable to remove Project member.')); } };
  const transferOwnership = async (userId: string, name: string) => { if (!projectId || !window.confirm(`Transfer Project ownership to ${name}? You will become a Project Admin.`)) return; const csrf = readCookie('__Host-linkary_csrf'); if (!csrf) return; try { await apiJson(`/api/projects/${projectId}/transfer-ownership`, { method: 'POST', headers: { 'x-csrf-token': csrf }, body: JSON.stringify({ userId }) }); setMessage('Project ownership transferred.'); await loadAdminData(); } catch (err) { setMessage(publicError(err, 'Only the current Project Owner can transfer ownership.')); } };
  return <div className="feature-page"><span className="section-label">PROJECT ACCESS</span><h1>Manage Projects</h1><p>Registered Projects own campaigns and attribution. People request roles to manage them.</p><form className="feature-form" onSubmit={(event) => { event.preventDefault(); void search(); }}><label>Find a registered Project<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Project name or @handle" minLength={2} required /></label><label>Requested role<select value={role} onChange={(event) => setRole(event.target.value)}><option value="marketing_manager">Campaign Manager</option><option value="analyst">Analyst</option><option value="viewer">Viewer</option></select></label><label>Why should this Project add you?<textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="Brief context for the Project Admin" maxLength={500} /></label><button className="button primary">Search Projects</button>{projects.map((project) => <div className="link-row" key={project.organization_id}><span><strong>{project.name}</strong><small>@{project.username} · registered Project</small></span><button className="button secondary" type="button" onClick={() => void request(project.organization_id)}>Request access</button></div>)}{message && <div className="form-error">{message}</div>}</form><section className="feature-form"><div><span className="section-label">MY REQUESTS</span><h2>Project access status</h2></div>{!myRequests.length ? <p>You have no Project access requests.</p> : myRequests.map((item) => <div className="link-row" key={item.id}><span><strong>{item.name}</strong><small>@{item.username} · {item.requested_role.replace('_', ' ')} · {item.status}</small></span>{item.status === 'submitted' && <button className="button secondary" type="button" onClick={() => void cancelRequest(item.id)}>Cancel</button>}</div>)}</section>{isProject && <><section className="feature-form"><div><span className="section-label">PROJECT ADMIN</span><h2>Access requests</h2></div>{!requests.length ? <p>No access requests waiting.</p> : requests.map((item) => <div className="link-row" key={item.id}><span><strong>{item.display_name || item.email || 'Linkary user'}</strong><small>{item.requested_role.replace('_', ' ')}{item.note ? ` · ${item.note}` : ''}</small></span><div className="link-actions"><button className="button primary" onClick={() => void review(item.id, 'approve')}>Approve</button><button className="button secondary" onClick={() => void review(item.id, 'reject')}>Reject</button></div></div>)}</section><section className="feature-form"><div><span className="section-label">PROJECT TEAM</span><h2>People with Project access</h2><p>Owners remain protected. Project Admins can manage non-admin operational roles.</p></div>{!members.length ? <p>Team members will appear here after access is granted.</p> : members.map((member) => <div className="link-row" key={member.user_id}><span><strong>{member.display_name || member.email || 'Linkary user'}</strong><small>{member.role.replace('_', ' ')}{member.billing_manager ? ' · billing manager' : ''}</small></span>{member.role === 'owner' ? <b>Owner</b> : <div className="link-actions"><select aria-label="Project role" value={member.role} onChange={(event) => void updateMember(member.user_id, event.target.value)}><option value="admin">Project Admin</option><option value="marketing_manager">Campaign Manager</option><option value="analyst">Analyst</option><option value="viewer">Viewer</option></select><button className="button secondary" type="button" onClick={() => void removeMember(member.user_id)}>Remove</button><button className="button secondary" type="button" onClick={() => void transferOwnership(member.user_id, member.display_name || member.email || 'this member')}>Transfer ownership</button></div>}</div>)}</section></>}</div>;
}

function FoundationPage({ name, profile }: { name: keyof typeof pageCopy; profile?: ProfileSummary }) {
  if (name === 'campaigns') return <CampaignPage />;
  if (name === 'tracking') return <TrackingPage />;
  if (name === 'settings' && profile) return <ProjectAccessPage profile={profile} />;
...
type OperationalProject = { id: string; name: string; status: string; verification_status: string; role: 'owner' | 'admin' | 'marketing_manager' | 'analyst' | 'viewer'; username?: string | null };
const canOperateProject = (project: OperationalProject | undefined) => Boolean(project && project.status === 'active' && project.verification_status === 'verified_x' && ['owner', 'admin', 'marketing_manager'].includes(project.role));
function ProjectContext({ project }: { project?: OperationalProject }) {
  if (!project) return <div className="notice danger"><strong>Select a Project</strong><span>Campaigns and attribution are always owned by a registered Project.</span></div>;
  const operational = canOperateProject(project);
  const role = project.role.replace('_', ' ');
  return <div className={operational ? 'notice' : 'notice danger'}><strong>{project.name} · {role}</strong><span>{project.status === 'active' && project.verification_status === 'verified_x' ? (operational ? 'Active and X-verified. Your role can create and manage campaign operations.' : 'Active and X-verified. Your role can view Project data, but cannot create campaigns or attribution records.') : 'This Project must be active and X-verified before Linkary can operate campaigns or attribution.'}</span></div>;
}

function CampaignPage() {
  type Campaign = { id: string; name: string; objective: string; budget_usd: number | null; status: string; created_at: string };
  type Org = OperationalProject;
  const [organizations, setOrganizations] = useState<Org[]>([]); const [organizationId, setOrganizationId] = useState(''); const [campaigns, setCampaigns] = useState<Campaign[]>([]); const [form, setForm] = useState({ name: '', source: 'Creators', outcome: 'Registrations', budgetUsd: '' }); const [step, setStep] = useState(1); const [message, setMessage] = useState('');
  ...
  async function create(event: React.FormEvent) { event.preventDefault(); const csrf = readCookie('__Host-linkary_csrf'); if (!csrf || !organizationId) return; try { await apiJson('/api/campaigns', { method: 'POST', headers: { 'x-csrf-token': csrf }, body: JSON.stringify({ organizationId, name: form.name, objective: `Source: ${form.source} · Result: ${form.outcome}`, budgetUsd: form.budgetUsd ? Number(form.budgetUsd) : undefined }) }); setForm({ name: '', source: 'Creators', outcome: 'Registrations', budgetUsd: '' }); setStep(1); const r = await apiJson<{ campaigns: Campaign[] }>(`/api/campaigns?organizationId=${encodeURIComponent(organizationId)}`); setCampaigns(r.campaigns); setMessage('Campaign created. Add an activity and tracking link next.'); } catch (err) { setMessage(publicError(err, 'Unable to create campaign.')); } }
  const project = organizations.find((item) => item.id === organizationId); const canWrite = canOperateProject(project);
  return <div className="feature-page"><span className="section-label">CAMPAIGNS</span><h1>Start measuring a campaign</h1><p>Track work already happening through your team, partners, creators, communities, or other channels.</p><label>Project workspace<select value={organizationId} onChange={(e) => setOrganizationId(e.target.value)} required><option value="">Select a Project</option>{organizations.map((org) => <option key={org.id} value={org.id}>{org.name}</option>)}</select></label><ProjectContext project={project} />{canWrite && <form className="feature-form" onSubmit={create}><div><span className="section-label">STEP {step} OF 3</span><h2>{step === 1 ? 'What are you tracking?' : step === 2 ? 'Where is it running?' : 'What result matters?'}</h2></div>{step === 1 && <label>Campaign name<input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. TGE creator activation" required /></label>}{step === 2 && <label>Primary source<select value={form.source} onChange={(e) => setForm({ ...form, source: e.target.value })}><option>Internal team</option><option>Agency</option><option>Creators</option><option>Community</option><option>Launchpad</option><option>External platform</option><option>Other</option></select></label>}{step === 3 && <><label>Result to measure<select value={form.outcome} onChange={(e) => setForm({ ...form, outcome: e.target.value })}><option>Registrations</option><option>Community joins</option><option>Purchases</option><option>Deposits</option><option>Content views</option><option>Custom outcome</option></select></label><label>Planned spend (USD)<input type="number" min="0" step="0.01" value={form.budgetUsd} onChange={(e) => setForm({ ...form, budgetUsd: e.target.value })} placeholder="Optional" /></label></>}<div className="profile-actions">{step > 1 && <button type="button" className="button secondary" onClick={() => setStep(step - 1)}>Back</button>}{step < 3 ? <button type="button" className="button primary" disabled={step === 1 && !form.name.trim()} onClick={() => setStep(step + 1)}>Continue</button> : <button className="button primary" disabled={!form.name.trim()}>Create campaign</button>}</div></form>}{message && <div className="form-error">{message}</div>}<section className="feature-form"><h2>Campaigns</h2>{!campaigns.length ? <p>No campaigns in this Project yet.</p> : campaigns.map((campaign) => <div className="link-row" key={campaign.id}><span><strong>{campaign.name}</strong><small>{campaign.objective || 'No objective set'} · {campaign.status}</small></span><b>{campaign.budget_usd === null ? '—' : `$${campaign.budget_usd.toLocaleString()}`}</b></div>)}</section></div>;
}

function TrackingPage() {
  type Org = OperationalProject; type Campaign = { id: string; name: string }; type Activity = { id: string; title: string; activity_type: string; destination_url: string | null; status: string };
  type TrackedLink = { id: string; code: string; destination_url: string; status: 'active' | 'paused' | 'archived'; clicks: number | string; created_at: string };
  type Conversion = { id: string; external_event_key: string; event_type: string; value_usd: number | null; source: string; attribution_confidence: string; occurred_at: string };
  const [organizations, setOrganizations] = useState<Org[]>([]); const [organizationId, setOrganizationId] = useState(''); const [campaigns, setCampaigns] = useState<Campaign[]>([]); const [campaignId, setCampaignId] = useState(''); const [activities, setActivities] = useState<Activity[]>([]); const [form, setForm] = useState({ title: '', type: 'creator_content', destinationUrl: '', plannedCostUsd: '' }); const [message, setMessage] = useState(''); const [link, setLink] = useState(''); const [outcome, setOutcome] = useState<{ conversions: number; value_usd: number; tracked_clicks: number } | null>(null); const [trackedLinks, setTrackedLinks] = useState<TrackedLink[]>([]); const [conversions, setConversions] = useState<Conversion[]>([]); const [conversion, setConversion] = useState({ trackedLinkId: '', eventType: 'registration', eventKey: '', valueUsd: '' });
  ...
  useEffect(() => { if (organizationId) apiJson<{ campaigns: Campaign[] }>(`/api/campaigns?organizationId=${encodeURIComponent(organizationId)}`).then((r) => { setCampaigns(r.campaigns); setCampaignId(r.campaigns[0]?.id || ''); }).catch(() => setMessage('Tracking is not available for this Project yet.')); }, [organizationId]);
  ...
  const loadOutcome = () => { if (campaignId) apiJson<{ summary: { conversions: number; value_usd: number; tracked_clicks: number } }>(`/api/campaign-outcomes?campaignId=${encodeURIComponent(campaignId)}`).then((r) => setOutcome(r.summary)).catch(() => setOutcome(null)); };
  const loadLinks = () => { if (campaignId) apiJson<{ links: TrackedLink[] }>(`/api/tracked-links?campaignId=${encodeURIComponent(campaignId)}`).then((r) => { setTrackedLinks(r.links); setConversion((current) => ({ ...current, trackedLinkId: current.trackedLinkId || r.links[0]?.id || '' })); }).catch(() => setTrackedLinks([])); };
  const loadConversions = () => { if (campaignId) apiJson<{ conversions: Conversion[] }>(`/api/conversions?campaignId=${encodeURIComponent(campaignId)}`).then((r) => setConversions(r.conversions)).catch(() => setConversions([])); };
  useEffect(() => { loadOutcome(); loadLinks(); loadConversions(); }, [campaignId]);
  ...
  async function track(activityId: string) { const csrf = readCookie('__Host-linkary_csrf'); if (!csrf) return; try { const r = await apiJson<{ url: string }>('/api/tracked-links', { method: 'POST', headers: { 'x-csrf-token': csrf }, body: JSON.stringify({ activityId }) }); setLink(r.url); loadLinks(); } catch (err) { setMessage(publicError(err, 'Add a destination URL before generating a tracking link.')); } }
  async function updateLinkStatus(linkId: string, status: TrackedLink['status']) { const csrf = readCookie('__Host-linkary_csrf'); if (!csrf) return; try { await apiJson(`/api/tracked-links/${encodeURIComponent(linkId)}/status`, { method: 'PATCH', headers: { 'x-csrf-token': csrf }, body: JSON.stringify({ status }) }); loadLinks(); } catch (err) { setMessage(publicError(err, 'Unable to update the tracking link.')); } }
  async function copyTrackingUrl(code: string) { const url = `${window.location.origin}/r/${code}`; try { await navigator.clipboard.writeText(url); setMessage('Tracking link copied.'); } catch { setLink(url); setMessage('Select and copy the tracking link below.'); } }
  async function recordConversion(event: React.FormEvent) { event.preventDefault(); const csrf = readCookie('__Host-linkary_csrf'); if (!csrf || !conversion.trackedLinkId) return; try { const result = await apiJson<{ duplicate?: boolean }>('/api/conversions', { method: 'POST', headers: { 'x-csrf-token': csrf }, body: JSON.stringify({ ...conversion, valueUsd: conversion.valueUsd ? Number(conversion.valueUsd) : undefined }) }); setConversion({ trackedLinkId: conversion.trackedLinkId, eventType: 'registration', eventKey: '', valueUsd: '' }); loadOutcome(); loadConversions(); setMessage(result.duplicate ? 'That evidence key was already recorded.' : 'Outcome recorded in the ledger.'); } catch (err) { setMessage(publicError(err, 'Unable to record this conversion.')); } }
  const project = organizations.find((item) => item.id === organizationId); const canWrite = canOperateProject(project);
  return <div className="feature-page"><span className="section-label">FIRST-PARTY ATTRIBUTION</span><h1>Activities & tracking</h1><p>Run anywhere. Track it in Linkary. Projects own the evidence and approved roles record the work.</p><label>Project<select value={organizationId} onChange={(e) => setOrganizationId(e.target.value)}><option value="">Select a Project</option>{organizations.map((org) => <option key={org.id} value={org.id}>{org.name}</option>)}</select></label><ProjectContext project={project} />{canWrite && <><form className="feature-form" onSubmit={create}><label>Campaign<select value={campaignId} onChange={(e) => setCampaignId(e.target.value)}><option value="">Select a campaign</option>{campaigns.map((campaign) => <option key={campaign.id} value={campaign.id}>{campaign.name}</option>)}</select></label><label>Activity<input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Creator post, community placement, or video" required /></label><label>Type<select value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })}><option value="creator_content">Creator content</option><option value="community_placement">Community placement</option><option value="website">Website</option><option value="video">Video</option><option value="other">Other</option></select></label><label>Destination URL<input type="url" value={form.destinationUrl} onChange={(e) => setForm({ ...form, destinationUrl: e.target.value })} placeholder="https://..." required /></label><button className="button primary" disabled={!campaignId || !form.title || !form.destinationUrl}>Create activity</button></form><form className="feature-form" onSubmit={recordConversion}><div><span className="section-label">OUTCOME LEDGER</span><h2>Record an outcome</h2></div><label>Tracked link<select value={conversion.trackedLinkId} onChange={(e) => setConversion({ ...conversion, trackedLinkId: e.target.value })}><option value="">Select a tracked link</option>{trackedLinks.map((item) => <option key={item.id} value={item.id}>{item.code} · {item.destination_url}</option>)}</select></label><label>Outcome type<input value={conversion.eventType} onChange={(e) => setConversion({ ...conversion, eventType: e.target.value })} placeholder="registration, sale, activation" required /></label><label>Evidence key<input value={conversion.eventKey} onChange={(e) => setConversion({ ...conversion, eventKey: e.target.value })} placeholder="Your unique CRM or order ID" required /></label><label>Attributed value (USD)<input type="number" min="0" step="0.01" value={conversion.valueUsd} onChange={(e) => setConversion({ ...conversion, valueUsd: e.target.value })} placeholder="Optional" /></label><button className="button primary" disabled={!conversion.trackedLinkId || !conversion.eventKey}>Record outcome</button></form></>}{message && <div className="form-error">{message}</div>}{outcome && <section className="real-metrics"><article><span>TRACKED CLICKS</span><strong>{outcome.tracked_clicks}</strong><small>Linkary measured</small></article><article><span>OUTCOMES</span><strong>{outcome.conversions}</strong><small>Recorded evidence</small></article><article><span>ATTRIBUTED VALUE</span><strong>${outcome.value_usd.toLocaleString()}</strong><small>Recorded outcomes</small></article></section>}<section className="feature-form"><h2>Tracked activities</h2>{!activities.length ? <p>No activities in the selected Project campaign yet.</p> : activities.map((activity) => <div className="link-row" key={activity.id}><span><strong>{activity.title}</strong><small>{activity.activity_type} · {activity.status}</small></span>{canWrite && <button className="button secondary" onClick={() => void track(activity.id)}>Create tracking link</button>}</div>)}{link && <div className="created-invite"><strong>Tracking link ready</strong><input value={link} readOnly onFocus={(e) => e.currentTarget.select()} /></div>}</section><section className="feature-form"><h2>Tracking links</h2>{!trackedLinks.length ? <p>Create a tracking link from an activity to start measuring clicks.</p> : trackedLinks.map((item) => <div className="link-row" key={item.id}><span><strong>{item.code} · {item.status}</strong><small>{item.destination_url} · {Number(item.clicks)} click{Number(item.clicks) === 1 ? '' : 's'}</small></span><div className="link-actions"><button type="button" className="button secondary" onClick={() => void copyTrackingUrl(item.code)}>Copy</button>{canWrite && item.status !== 'active' && <button type="button" className="button secondary" onClick={() => void updateLinkStatus(item.id, 'active')}>Activate</button>}{canWrite && item.status === 'active' && <button type="button" className="button secondary" onClick={() => void updateLinkStatus(item.id, 'paused')}>Pause</button>}{canWrite && item.status !== 'archived' && <button type="button" className="button secondary" onClick={() => void updateLinkStatus(item.id, 'archived')}>Archive</button>}</div></div>)}</section><section className="feature-form"><h2>Outcome ledger</h2>{!conversions.length ? <p>No outcomes recorded for this campaign yet.</p> : conversions.map((item) => <div className="link-row" key={item.id}><span><strong>{item.event_type}</strong><small>{item.attribution_confidence} evidence · {new Date(item.occurred_at).toLocaleDateString()} · {item.external_event_key}</small></span><b>{item.value_usd === null ? '—' : `$${Number(item.value_usd).toLocaleString()}`}</b></div>)}</section></div>;
}
...
function ProfileEditor({ profile }: { profile: ProfileSummary }) {
  ...
  const [analytics, setAnalytics] = useState<{ linkClicks: number } | null>(null);
  const [message, setMessage] = useState('');
  const csrf = () => readCookie('__Host-linkary_csrf');
  ...
  useEffect(() => { apiJson<{ linkClicks: number }>(`/api/profiles/${encodeURIComponent(profile.id)}/analytics`).then(setAnalytics).catch(() => setAnalytics(null)); }, [profile.id]);
  ...
  async function save(event: React.FormEvent) { event.preventDefault(); const csrf = readCookie('__Host-linkary_csrf'); if (!csrf) { setMessage('Please sign in again.'); return; } try { await apiJson(`/api/profiles/${encodeURIComponent(profile.id)}`, { method: 'PATCH', headers: { 'x-csrf-token': csrf }, body: JSON.stringify(form) }); const saved = await apiJson<{ profile: { displayName: string; bio: string | null; seoTitle: string | null; seoDescription: string | null } }>(`/api/profiles/${encodeURIComponent(profile.id)}`); setForm({ displayName: saved.profile.displayName, bio: saved.profile.bio || '', seoTitle: saved.profile.seoTitle || '', seoDescription: saved.profile.seoDescription || '' }); setMessage('Profile saved.'); } catch (err) { setMessage(publicError(err, 'Unable to save profile.')); } }
...
function AdminPage() {
  ...
  const [creditOwners, setCreditOwners] = useState<{ profiles: Array<{ id: string; display_name: string; username: string }>; organizations: Array<{ id: string; name: string }> }>({ profiles: [], organizations: [] });
  const [creditForm, setCreditForm] = useState({ ownerType: 'profile', ownerId: '', amount: '', reason: '' });
  const [busy, setBusy] = useState<string | null>(null);
  const [error, setError] = useState('');
  ...
  async function load() {
    try {
      const [queue, verification, userList, owners] = await Promise.all([
        ...
        apiJson<{ profiles: Array<{ id: string; display_name: string; username: string }>; organizations: Array<{ id: string; name: string }> }>('/api/admin/invite-credit-owners'),
      ]);
      setClaims(queue.claims); setSetting(verification); setUsers(userList.users); setCreditOwners(owners); setError('');
  ...
  async function adjustCredits(event: React.FormEvent) { event.preventDefault(); const csrf = readCookie('__Host-linkary_csrf'); if (!csrf) return; try { await apiJson('/api/admin/invite-credits/adjust', { method: 'POST', headers: { 'x-csrf-token': csrf }, body: JSON.stringify({ ownerType: creditForm.ownerType, ownerId: creditForm.ownerId, amount: Number(creditForm.amount), reason: creditForm.reason }) }); setCreditForm({ ownerType: 'profile', ownerId: '', amount: '', reason: '' }); setError('Invite credits adjusted and audited.'); } catch (err) { setError(publicError(err, 'Unable to adjust credits.')); } }
  ...
  return (
    <div className="admin-review-page">
      ...
      <form className="feature-form" onSubmit={adjustCredits}><div><span className="section-label">INVITE CREDITS</span><h2>Adjust invite credits</h2><p>Every adjustment requires a reason and is recorded in the audit ledger.</p></div><label>Owner type<select value={creditForm.ownerType} onChange={(event) => setCreditForm({ ownerType: event.target.value, ownerId: '', amount: creditForm.amount, reason: creditForm.reason })}><option value="profile">Creator profile</option><option value="organization">Project workspace</option></select></label><label>Owner<select value={creditForm.ownerId} onChange={(event) => setCreditForm({ ...creditForm, ownerId: event.target.value })} required><option value="">Select an owner</option>{creditForm.ownerType === 'profile' ? creditOwners.profiles.map((owner) => <option key={owner.id} value={owner.id}>{owner.display_name} /{owner.username}</option>) : creditOwners.organizations.map((owner) => <option key={owner.id} value={owner.id}>{owner.name}</option>)}</select></label><label>Credit adjustment<input type="number" value={creditForm.amount} onChange={(event) => setCreditForm({ ...creditForm, amount: event.target.value })} placeholder="e.g. 10 or -1" step="1" required /></label><label>Reason<textarea value={creditForm.reason} onChange={(event) => setCreditForm({ ...creditForm, reason: event.target.value })} maxLength={240} placeholder="Why is this credit adjustment needed?" required /></label><button className="button primary" type="submit" disabled={!creditForm.ownerId || !creditForm.amount || !creditForm.reason.trim()}>Save audited adjustment</button></form>
    </div>
  );
}
