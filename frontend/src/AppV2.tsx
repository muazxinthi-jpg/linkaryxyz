...
function TrackingPage() {
  ...
  async function track(activityId: string) { const csrf = readCookie('__Host-linkary_csrf'); if (!csrf) return; try { const r = await apiJson<{ url: string }>('/api/tracked-links', { method: 'POST', headers: { 'x-csrf-token': csrf }, body: JSON.stringify({ activityId }) }); setLink(r.url); const refreshed = await apiJson<{ links: Array<{ id: string; code: string; destination_url: string }> }>(`/api/tracked-links?campaignId=${encodeURIComponent(campaignId)}`); setTrackedLinks(refreshed.links); setConversion((current) => ({ ...current, trackedLinkId: current.trackedLinkId || refreshed.links[0]?.id || '' })); } catch (err) { setMessage(publicError(err, 'Add a destination URL before generating a tracking link.')); } }
