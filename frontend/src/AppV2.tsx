...
function ProjectAccessPage({ profile }: { profile: ProfileSummary }) { const [query, setQuery] = useState(''); const [projects, setProjects] = useState<Array<{ organization_id: string; name: string; username: string }>>([]); const [role, setRole] = useState('marketing_manager'); const [message, setMessage] = useState(''); const [requests, setRequests] = useState<Array<{ id: string; requested_role: string; note: string; display_name: string; email: string | null }>>([]); const search = async () => { try { const r = await apiJson<{ projects: typeof projects }>(`/api/projects/search?query=${encodeURIComponent(query)}`); setProjects(r.projects); } catch (err) { setMessage(publicError(err, 'Unable to search registered Projects.')); } }; const request = async (organizationId: string) => { const csrf = readCookie('__Host-linkary_csrf'); if (!csrf) return; try { await apiJson(`/api/projects/${organizationId}/access-requests`, { method: 'POST', headers: { 'x-csrf-token': csrf }, body: JSON.stringify({ role }) }); setMessage('Access request sent to the Project Admins.'); } catch (err) { setMessage(publicError(err, 'Unable to send access request.')); } }; const loadRequests = async () => { if (!profile.organization_id) return; try { const r = await apiJson<{ requests: typeof requests }>(`/api/projects/${profile.organization_id}/access-requests`); setRequests(r.requests); } catch { setRequests([]); } }; useEffect(() => { void loadRequests(); }, [profile.organization_id]); const review = async (id: string, decision: 'approve' | 'reject') => { const csrf = readCookie('__Host-linkary_csrf'); if (!csrf) return; await apiJson(`/api/projects/access-requests/${id}/${decision}`, { method: 'POST', headers: { 'x-csrf-token': csrf } }); await loadRequests(); }; return <div className="feature-page"><span className="section-label">PROJECT ACCESS</span><h1>Manage Projects</h1><p>Registered Projects own campaigns and attribution. People request roles to manage them.</p><form className="feature-form" onSubmit={(event) => { event.preventDefault(); void search(); }}><label>Find a registered Project<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Project name or @handle" minLength={2} required /></label><label>Requested role<select value={role} onChange={(event) => setRole(event.target.value)}><option value="marketing_manager">Campaign Manager</option><option value="analyst">Analyst</option><option value="viewer">Viewer</option></select></label><button className="button primary">Search Projects</button>{projects.map((project) => <div className="link-row" key={project.organization_id}><span><strong>{project.name}</strong><small>@{project.username} · registered Project</small></span><button className="button secondary" type="button" onClick={() => void request(project.organization_id)}>Request access</button></div>)}{message && <div className="form-error">{message}</div>}</form>{profile.profile_type === 'project' && <section className="feature-form"><div><span className="section-label">PROJECT ADMIN</span><h2>Access requests</h2></div>{!requests.length ? <p>No access requests waiting.</p> : requests.map((item) => <div className="link-row" key={item.id}><span><strong>{item.display_name || item.email || 'Linkary user'}</strong><small>{item.requested_role.replace('_', ' ')}{item.note ? ` · ${item.note}` : ''}</small></span><div className="link-actions"><button className="button primary" onClick={() => void review(item.id, 'approve')}>Approve</button><button className="button secondary" onClick={() => void review(item.id, 'reject')}>Reject</button></div></div>)}</section>}</div>; }
