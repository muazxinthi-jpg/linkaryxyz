...
function CampaignPage() {
  ...
  return <div className="feature-page"><span className="section-label">CAMPAIGNS</span><h1>Campaign operations</h1><p>Projects own campaigns. People manage them through verified Project roles.</p><label>Project workspace<select value={organizationId} onChange={(e) => setOrganizationId(e.target.value)} required><option value="">Select a Project</option>{organizations.map((org) => <option key={org.id} value={org.id}>{org.name}</option>)}</select></label><ProjectContext project={project} />{canWrite && <form className="feature-form" onSubmit={create}><label>Campaign name<input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Summer Growth" required /></label><label>Objective<textarea value={form.objective} onChange={(e) => setForm({ ...form, objective: e.target.value })} placeholder="What outcome should this campaign drive?" /></label><label>Budget (USD)<input type="number" min="0" step="0.01" value={form.budgetUsd} onChange={(e) => setForm({ ...form, budgetUsd: e.target.value })} placeholder="20000" /></label><button className="button primary" disabled={!form.name.trim()}>Create campaign</button></form>}{message && <div className="form-error">{message}</div>}<section className="feature-form"><h2>Campaigns</h2>{!campaigns.length ? <p>No campaigns in this Project yet.</p> : campaigns.map((campaign) => <div className="link-row" key={campaign.id}><span><strong>{campaign.name}</strong><small>{campaign.objective || 'No objective set'} · {campaign.status}</small></span><b>{campaign.budget_usd === null ? '—' : `$${campaign.budget_usd.toLocaleString()}`}</b></div>)}</section></div>;
}
