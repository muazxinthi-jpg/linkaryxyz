...
function ProfileEditor({ profile }: { profile: ProfileSummary }) {
  ...
  async function save(event: React.FormEvent) { event.preventDefault(); const csrf = readCookie('__Host-linkary_csrf'); if (!csrf) { setMessage('Please sign in again.'); return; } try { await apiJson(`/api/profiles/${encodeURIComponent(profile.id)}`, { method: 'PATCH', headers: { 'x-csrf-token': csrf }, body: JSON.stringify(form) }); const saved = await apiJson<{ display_name: string; bio: string | null; seo_title: string | null; seo_description: string | null }>(`/api/profiles/${encodeURIComponent(profile.id)}`); setForm({ displayName: saved.display_name, bio: saved.bio || '', seoTitle: saved.seo_title || '', seoDescription: saved.seo_description || '' }); setMessage('Profile saved.'); } catch (err) { setMessage(publicError(err, 'Unable to save profile.')); } }
