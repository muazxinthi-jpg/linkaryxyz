...
type Member = { user_id: string; role: Role; billing_manager: number; display_name: string; username: string | null };
type Candidate = { id: string; display_name: string; username: string | null };
...
type IncomingRequest = { id: string; requested_role: RequestableRole; note: string; created_at: string; display_name: string; username: string | null };
...
export default function ProjectExperienceBeta({ me, status }: { me: ProductMe; status: ProductStatus }) {
  ...
  async function removeMember(member: Member) {
    if (!projectId || !window.confirm(`Remove ${member.display_name || member.username || 'this member'} from ${project?.name}? They will lose Project access immediately.`)) return;
    const token = csrf(); if (!token) return;
