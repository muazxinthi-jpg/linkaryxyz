  ...
  return <ProductWorkspace me={me} status={status} profile={profile as ProductProfile} onProfileChange={changeProfile}>
    <div className="ops-stack partner-directory partner-discovery-v1">
      ...
      {!firstProject ? <section className="ops-empty prominent"><div className="ops-empty-icon">◎</div><h2>Connect a Project first</h2><p>Partner discovery is a Project workspace. Create or join a verified Project before building a shortlist.</p><a className="ops-button primary" href="/settings">Manage Projects</a></section> : profile.profile_type !== 'project' ? <section className="ops-callout verification"><div><span className="ops-kicker">PROJECT WORKSPACE REQUIRED</span><h3>Switch “View as” to a Project</h3><p>Discovery, private relationship memory and collaboration inquiries belong to Projects.</p></div></section> : <>
        ...
        <section className="partner-summary-strip" aria-label="How Project partner discovery works">
          <div className="partner-principle discover"><span className="partner-principle-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><circle cx="10.8" cy="10.8" r="6.2"/><path d="m15.4 15.4 4.2 4.2M8.4 10.8h4.8M10.8 8.4v4.8"/></svg></span><span className="partner-principle-copy"><strong>Discover</strong><span>Find published Creators and Community Managers with canonical Linkary identities.</span></span></div>
          <div className="partner-principle remember"><span className="partner-principle-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M4.5 7.5A8 8 0 1 1 4 14M4.5 4v4h4M12 7v5l3.5 2"/></svg></span><span className="partner-principle-copy"><strong>Remember</strong><span>See your Project's prior campaigns, tracked clicks, verified outcomes and exact Communities used.</span></span></div>
          <div className="partner-principle work-again"><span className="partner-principle-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M19 8V4m0 0h-4m4 0-3.1 3.1A7 7 0 1 0 19 12"/></svg></span><span className="partner-principle-copy"><strong>Work again</strong><span>Previously activated relationships can start a fresh inquiry without rewriting historical proof.</span></span></div>
        </section>
        ...
        <section className="ops-section partner-discovery-workspace" aria-label="Partner search and results">
          <div className="partner-toolbar"><nav className="ops-tabs" aria-label="Partner type"><button type="button" aria-pressed={type === 'creator'} className={type === 'creator' ? 'active' : ''} onClick={() => setType('creator')}>Creators</button><button type="button" aria-pressed={type === 'community_manager'} className={type === 'community_manager' ? 'active' : ''} onClick={() => setType('community_manager')}>Community Managers</button></nav><form onSubmit={(event) => { event.preventDefault(); void loadPartners(); }}><input type="search" aria-label={type === 'creator' ? 'Search creators by name, handle, or bio' : 'Search Community Managers by name, Community, or handle'} value={search} onChange={(event) => setSearch(event.target.value)} placeholder={type === 'creator' ? 'Search creator, @handle or bio' : 'Search manager, Community or @handle'} /><button type="submit" className="ops-button small">Search</button></form></div>
            ...
            <div className="partner-discovery-filters">
              ...
              {type === 'community_manager' && <>
                <label className="partner-number-filter">Minimum combined audience<input type="number" min="0" value={minAudience} onChange={(event) => setMinAudience(event.target.value)} placeholder="Any" /></label>
                <label className="partner-number-filter">Minimum Communities<input type="number" min="0" value={minCommunities} onChange={(event) => setMinCommunities(event.target.value)} placeholder="Any" /></label>
              </>}
            </div>
            <div className="partner-filter-bottom">
              <div className="partner-active-filters" aria-label="Selected filters">
                {verifiedOnly && <button type="button" className="partner-filter-chip" onClick={() => setVerifiedOnly(false)}>{type === 'creator' ? 'Verified creators' : 'Verified Community'} <span aria-hidden="true">×</span></button>}
                {openOnly && <button type="button" className="partner-filter-chip" onClick={() => setOpenOnly(false)}>{type === 'creator' ? 'Open to collaboration' : 'Open to campaigns'} <span aria-hidden="true">×</span></button>}
                {type === 'community_manager' && minAudience && <button type="button" className="partner-filter-chip" onClick={() => setMinAudience('')}>Audience ≥ {compact(Number(minAudience))} <span aria-hidden="true">×</span></button>}
                {type === 'community_manager' && minCommunities && <button type="button" className="partner-filter-chip" onClick={() => setMinCommunities('')}>Communities ≥ {minCommunities} <span aria-hidden="true">×</span></button>}
              </div>
          ...
          {loading ? <div className="ops-loading">Loading partners...</div> : !partners.length ? <div className="ops-empty"><div className="ops-empty-icon">◇</div><h3>No matching partners yet</h3><p>Try widening the filters. Discovery only shows real published Creator profiles and public Community Manager portfolios.</p></div> : <div className="partner-grid">{partners.map((partner) => {
            ...
            return <article className="partner-card" key={`${partner.kind}:${partner.id}`}>
              <div className="partner-card-head"><Avatar src={partner.avatar_url} name={partner.display_name} /><div><strong>{partner.display_name}</strong><span>{partner.kind === 'creator' ? `@${partner.username}` : partner.headline || 'Community Manager'}</span></div><span className={`partner-verify ${partner.kind === 'creator' ? (partner.verified ? 'verified' : 'unverified') : (partner.verified_communities > 0 ? 'verified' : 'unverified')}`}>{partner.kind === 'creator' ? (partner.verified ? 'Verified' : 'Listed') : `${partner.verified_communities} verified`}</span><button type="button" className="partner-save-button" aria-label={`Save ${partner.display_name} to Project shortlist`} title={`Shortlist ${partner.display_name}`} disabled={!canManage(project) || savingId === partner.id} onClick={() => void shortlist(partner)}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6.5 4.75h11a1 1 0 0 1 1 1v14l-6.5-4-6.5 4v-14a1 1 0 0 1 1-1Z" /></svg></button></div>
