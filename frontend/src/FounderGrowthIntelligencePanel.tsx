...
export default function FounderGrowthIntelligencePanel({ organizationId }: { organizationId: string }) {
  ...
  const strongestPartner = useMemo(() => data?.partners.filter((item) => item.outcomes > 0 || item.attributed_value_usd > 0).sort((a, b) => b.attributed_value_usd - a.attributed_value_usd || b.outcomes - a.outcomes)[0] || null, [data]);
  ...
  return <section className="fgi-shell" aria-label="Founder Growth Intelligence">
    ...
    <div className="fgi-signal-grid">
      ...
      <article><span>STRONGEST PARTNER</span><strong>{strongestPartner?.label || 'Not enough partner evidence'}</strong><small>{strongestPartner ? `${compact(strongestPartner.tracked_clicks)} clicks · ${compact(strongestPartner.outcomes)} outcomes` : 'Assign exact Creators or Communities to activities.'}</small></article>
    ...
    {!rows.length ? <div className="fgi-empty">No {tab} have enough recorded evidence yet.</div> : <div className="fgi-list">
      {rows.map((row) => {
        ...
        const meta = campaign ? `${human(campaign.source_type)} · ${human(campaign.status)} · Budget ${campaign.budget_usd === null ? 'not set' : money(campaign.budget_usd)}` : activity ? `${activity.campaign_name} · ${human(activity.channel)} · ${activity.partner_display_name || 'Unassigned'}` : group ? `${group.activities} activit${group.activities === 1 ? 'y' : 'ies'}${group.handle ? ` · @${group.handle}` : ''}` : '';
        return <article className="fgi-row" key={key}><div className="fgi-row-head"><div><strong>{title}</strong><span>{meta}</span></div>{group && <small>Spend: activity-attached only</small>}</div><MetricStrip value={row as Performance} /></article>;
      })}
    </div>}

    <details className="fgi-methodology"><summary>How these metrics are calculated</summary><p>{data.methodology.manual_social_metrics}</p><p>{data.methodology.unique_clicks}</p><p>{data.methodology.partner_channel_spend}</p><p>{data.methodology.missing_metrics}</p></details>
  </section>;
}
