...
function compatibilityStatus(me: ProductMe): ProductStatus {
  ...
  return {
    user: { id: userId, displayName: me.user?.displayName || 'Superadmin', email: null },
    profiles: [{ id: 'superadmin-console', profile_type: 'creator', username: 'superadmin', display_name: 'Superadmin', visibility: 'private', organization_id: null }],
  };
}
...
export default function SuperadminApp({ me }: { me: ProductMe }) {
  ...
  if (location.pathname === '/admin/promotion-review') return <SuperadminWorkspace me={me}><AdminPromotionReviewExperience /></SuperadminWorkspace>;
