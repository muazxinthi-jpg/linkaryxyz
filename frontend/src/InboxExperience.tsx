...
type ActionFilter = 'all' | Action['kind'];
...
function InboxActionIcon({ kind }: { kind: Action['kind'] }) {
  const paths: Record<Action['kind'], React.ReactNode> = {
    project_access: <><rect x="3.5" y="6" width="17" height="14" rx="2"/><path d="M8 6V4h8v2M3.5 11h17M10 11v2h4v-2"/></>,
    opportunity_application: <><path d="M7 3.5h7l4 4V20H7a2 2 0 0 1-2-2V5.5a2 2 0 0 1 2-2Z"/><path d="M14 3.5v5h5M8.5 13h7M8.5 16h7"/></>,
    collaboration_inquiry: <><path d="M20 11.5a7.5 7.5 0 0 1-7.5 7.5H6l-3 2v-6.5A7.5 7.5 0 1 1 20 11.5Z"/><path d="M8 11.5h.01M12 11.5h.01M16 11.5h.01"/></>,
  };
  return <span className={`inbox-icon ${kind}`} aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">{paths[kind]}</svg></span>;
}
