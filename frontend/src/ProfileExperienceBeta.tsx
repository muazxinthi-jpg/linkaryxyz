...
export default function ProfileExperienceBeta({ me, status }: { me: ProductMe; status: ProductStatus }) {
  ...
  async function load() {
    ...
    try {
      ...
      const enabled = blockResult.blocks.filter((block) => block.enabled);
      const inferredX = enabled.find((block) => inferSocialPlatform(block) === 'x');
      setAnalytics({
        linkClicks: analyticsResult.linkClicks || 0,
        sections: analyticsResult.sections ?? enabled.length,
