...
import OnchainAttributionPanel from './OnchainAttributionPanel';
...
type TrackedLink = {
  ...
  destination_url: string; effective_destination_url?: string | null; utm_source?: string | null; utm_medium?: string | null; utm_campaign?: string | null; utm_content?: string | null; utm_term?: string | null;
  status: TrackingStatus; clicks: number; last_click_at: string | null; created_at: string;
};
type Outcome = {
...
export default function TrackingExperience({ me, status }: { me: ProductMe; status: ProductStatus }) {
  ...
  const [tab, setTab] = useState<'activities' | 'links' | 'outcomes' | 'onchain'>('activities');
  ...
  async function createTrackingLink(activityId: string) {
    const token = cookie(); if (!token) return;
    try { const result = await api<{ url: string }>('/api/tracked-links', { method: 'POST', headers: { 'x-csrf-token': token }, body: JSON.stringify({ activityId, utmTerm: utmTerm.trim() || undefined }) }); await navigator.clipboard.writeText(result.url).catch(() => undefined); setCopied(result.url); setUtmTerm(''); setTab('links'); await loadCampaignData(); window.setTimeout(() => setCopied(''), 2200); }
