...
export default function TrackingExperience({ me, status }: { me: ProductMe; status: ProductStatus }) {
  ...
  async function loadCampaignData() {
    ...
    try {
      ...
      const [activityResult, linkResult, summaryResult, outcomeResult] = await Promise.all([
        ...
        api<{ conversions: Outcome[] }>(`/api/conversions?${outcomeQuery.toString()}`),
      ]);
  ...
  async function createActivity(event: React.FormEvent) {
    ...
    try {
      ...
      setActivityForm({ title: '', type: 'creator_content', destinationUrl: '', plannedCostUsd: '', partnerMode: 'none', creatorProfileId: '', managerId: '', assetId: '' });
      setShowActivity(false); setMessage(partner ? 'Activity added and linked to the exact Linkary partner.' : 'Activity added. You can assign a Linkary partner at any time.'); await loadCampaignData();
  ...
  async function recordOutcome(event: React.FormEvent) {
    event.preventDefault(); const token = cookie(); if (!token || !outcomeForm.trackedLinkId) return;
    try { const result = await api<{ duplicate?: boolean }>('/api/conversions', { method: 'POST', headers: { 'x-csrf-token': token }, body: JSON.stringify({ trackedLinkId: outcomeForm.trackedLinkId, eventType: outcomeForm.eventType.trim(), eventKey: outcomeForm.eventKey.trim(), valueUsd: outcomeForm.valueUsd ? Number(outcomeForm.valueUsd) : undefined }) }); setOutcomeForm({ trackedLinkId: outcomeForm.trackedLinkId, eventType: 'registration', eventKey: '', valueUsd: '' }); setShowOutcome(false); setMessage(result.duplicate ? 'That external outcome ID already exists. The original outcome was kept.' : 'Outcome recorded as evidence.'); await loadCampaignData(); }
  ...
  if (filters.search) csvParams.set('search', filters.search); if (filters.source) csvParams.set('source', filters.source); if (filters.confidence) csvParams.set('confidence', filters.confidence);
  ...
  return <ProductWorkspace me={me} status={status} profile={profile as ProductProfile} onProfileChange={changeProfile}>
    <div className="ops-stack tracking-v2">
      <div className="ops-heading-row"><div><span className="ops-kicker">EVIDENCE</span><h1>Track campaign evidence</h1><p>Bind each activity to the exact Creator or Telegram Community, then connect traffic and outcomes back to that work.</p></div>{campaignId && writable(project) && <div className="ops-heading-actions"><button className="ops-button secondary" onClick={() => setShowActivity(true)}>+ Activity</button><button className="ops-button primary" onClick={() => setShowOutcome(true)} disabled={!links.length}>+ Outcome</button></div>}</div>
      ...
      {campaignId && <>
        <nav className="ops-tabs"><button className={tab === 'activities' ? 'active' : ''} onClick={() => setTab('activities')}>Activities <span>{activities.length}</span></button><button className={tab === 'links' ? 'active' : ''} onClick={() => setTab('links')}>Tracking links <span>{links.length}</span></button><button className={tab === 'outcomes' ? 'active' : ''} onClick={() => setTab('outcomes')}>Outcomes <span>{outcomes.length}</span></button></nav>
        {loading ? <div className="ops-loading">Loading campaign evidence...</div> : tab === 'activities' ? <section className="ops-section"><div className="ops-section-title"><div><h2>Activities and partners</h2><p>Record the distribution work and bind it to the exact Linkary identity used.</p></div>{writable(project) && <button className="ops-button secondary" onClick={() => setShowActivity(true)}>+ Add activity</button>}</div>{!activities.length ? <div className="ops-empty"><h3>No activities yet</h3><p>Add a creator post, Community placement, video, website or other activity.</p>{writable(project) && <button className="ops-button secondary" onClick={() => setShowActivity(true)}>Add first activity</button>}</div> : <div className="tracking-activity-list">{activities.map((activity) => <article className="tracking-activity-card" key={activity.id}><div className="tracking-activity-main"><div className="tracking-activity-head"><span className="ops-type-chip">{human(activity.activity_type)}</span><span className={`ops-status status-${activity.status}`}>{human(activity.status)}</span></div><h3>{activity.title}</h3><a className="tracking-destination" href={activity.destination_url || undefined} target="_blank" rel="noreferrer">{activity.destination_url || 'No destination URL'}</a><PartnerIdentity activity={activity} /></div><div className="tracking-activity-side"><div><span>PLANNED COST</span><strong>{money(activity.planned_cost_usd)}</strong></div>{writable(project) && <div className="tracking-activity-actions"><button className="ops-button small" onClick={() => openAssignment(activity)}>{activity.partner_kind ? 'Change partner' : 'Assign partner'}</button>{activity.partner_kind && <button className="ops-button small ghost" onClick={() => void clearAssignment(activity)}>Remove partner</button>}<button className="ops-button small primary" onClick={() => void createTrackingLink(activity.id)} disabled={!activity.destination_url}>Create tracking link</button></div>}</div></article>)}</div>}</section> : tab === 'links' ? <section className="ops-section"><div className="ops-section-title"><div><h2>Tracking links</h2><p>Share these URLs instead of raw destination links.</p></div></div>{!links.length ? <div className="ops-empty"><h3>No tracking links yet</h3><p>Create one from an activity to start measuring attributable traffic.</p><button className="ops-button secondary" onClick={() => setTab('activities')}>View activities</button></div> : <div className="ops-link-grid">{links.map((link) => <article className={`ops-link-card ${link.status}`} key={link.id}><div className="ops-link-head"><div><span className={`ops-status status-${link.status}`}>{human(link.status)}</span><strong>{link.activity_title || 'Campaign link'}</strong></div><span>{link.clicks.toLocaleString()} clicks</span></div><div className="ops-link-url"><input readOnly value={link.url} onFocus={(e) => e.currentTarget.select()} /><button onClick={() => void copyLink(link)}>{copied === link.id ? 'Copied' : 'Copy'}</button></div><div className="ops-link-destination"><span>DESTINATION</span><a href={link.destination_url} target="_blank" rel="noreferrer">{link.destination_url} ↗</a></div><div className="ops-link-footer"><span>Last click: {date(link.last_click_at)}</span>{writable(project) && <div>{link.status === 'active' ? <button onClick={() => void setLinkStatus(link, 'paused')}>Pause</button> : link.status === 'paused' ? <button onClick={() => void setLinkStatus(link, 'active')}>Reactivate</button> : null}{link.status !== 'archived' && <button onClick={() => void setLinkStatus(link, 'archived')}>Archive</button>}</div>}</div></article>)}</div>}</section> : <section className="ops-section"><div className="ops-section-title"><div><h2>Outcome Ledger</h2><p>Every outcome keeps its evidence source and confidence level.</p></div><a className="ops-button secondary" href={`/api/conversions?${csvParams.toString()}`}>Export CSV</a></div><div className="ops-filters"><input value={filters.search} onChange={(e) => setFilters({ ...filters, search: e.target.value })} placeholder="Search outcome ID or type" /><select value={filters.source} onChange={(e) => setFilters({ ...filters, source: e.target.value })}><option value="">All sources</option><option value="manual">Manual</option><option value="linkary_tracked">Linkary tracked</option><option value="telegram_verified">Telegram verified</option><option value="provider_verified">Provider verified</option></select><select value={filters.confidence} onChange={(e) => setFilters({ ...filters, confidence: e.target.value })}><option value="">All confidence</option><option value="manual">Manual</option><option value="tracked">Tracked</option><option value="correlated">Correlated</option><option value="verified">Verified</option></select><button className="ops-button small" onClick={() => void loadCampaignData()}>Apply</button></div>{!outcomes.length ? <div className="ops-empty"><h3>No outcomes recorded yet</h3><p>Outcomes appear here when they are recorded or received through Linkary attribution.</p>{writable(project) && links.length > 0 && <button className="ops-button secondary" onClick={() => setShowOutcome(true)}>Record first outcome</button>}</div> : <div className="ops-outcome-table"><div className="ops-outcome-header"><span>Outcome</span><span>Activity</span><span>Evidence</span><span>Value</span><span>Date</span></div>{outcomes.map((outcome) => <article key={outcome.id}><div><strong>{human(outcome.event_type)}</strong><small>{outcome.external_event_key}</small></div><div><strong>{outcome.activity_title || 'Campaign'}</strong><small>{human(outcome.activity_type)}</small></div><div><span className={`ops-confidence confidence-${outcome.attribution_confidence}`}>{human(outcome.attribution_confidence)}</span><small>{human(outcome.source)}</small></div><strong>{money(outcome.value_usd)}</strong><span>{date(outcome.occurred_at)}</span></article>)}</div>}</section>}
      </>}

      {showActivity && writable(project) && <div className="ops-modal-backdrop" onMouseDown={(e) => { if (e.currentTarget === e.target) setShowActivity(false); }}><form className="ops-modal tracking-assignment-modal" onSubmit={createActivity}><div className="ops-modal-head"><div><span className="ops-kicker">NEW ACTIVITY</span><h2>Add campaign activity</h2></div><button type="button" onClick={() => setShowActivity(false)}>×</button></div><label>Activity name<input value={activityForm.title} onChange={(e) => setActivityForm({ ...activityForm, title: e.target.value })} placeholder="Example: Creator launch thread" required /></label><div className="ops-field-grid two"><label>Type<select value={activityForm.type} onChange={(e) => setActivityForm({ ...activityForm, type: e.target.value })}><option value="creator_content">Creator content</option><option value="community_placement">Community placement</option><option value="website">Website</option><option value="video">Video</option><option value="other">Other</option></select></label><label>Planned cost, optional<input type="number" min="0" step="0.01" value={activityForm.plannedCostUsd} onChange={(e) => setActivityForm({ ...activityForm, plannedCostUsd: e.target.value })} placeholder="500" /></label></div><label>Destination URL<input type="url" value={activityForm.destinationUrl} onChange={(e) => setActivityForm({ ...activityForm, destinationUrl: e.target.value })} placeholder="https://project.xyz/signup" required /></label><div className="tracking-partner-picker"><div><span className="ops-kicker">LINKARY PARTNER, OPTIONAL</span><h3>Who is doing this activity?</h3><p>Choose the exact identity when the activity belongs to a Linkary Creator or Telegram Community.</p></div><label>Partner type<select value={activityForm.partnerMode} onChange={(e) => { const mode = e.target.value as PartnerMode; setActivityForm({ ...activityForm, partnerMode: mode, creatorProfileId: '', managerId: '', assetId: '' }); }}><option value="none">No Linkary partner yet</option><option value="creator">Creator</option><option value="community">Telegram Community</option></select></label>{partnersLoading ? <small>Loading Linkary partners...</small> : activityForm.partnerMode === 'creator' ? <label>Creator<select value={activityForm.creatorProfileId} onChange={(e) => setActivityForm({ ...activityForm, creatorProfileId: e.target.value })}><option value="">Select Creator</option>{creators.map((creator) => <option key={creator.profile_id} value={creator.profile_id}>{creator.display_name}{creator.x_handle ? ` (@${creator.x_handle.replace(/^@/, '')})` : ''}{creator.verified ? ' · Verified' : ''}</option>)}</select></label> : activityForm.partnerMode === 'community' ? <><label>Community Manager<select value={activityForm.managerId} onChange={(e) => { const managerId = e.target.value; setActivityForm({ ...activityForm, managerId, assetId: '' }); void loadCommunityAssets(managerId); }}><option value="">Select Community Manager</option>{communityManagers.map((manager) => <option key={manager.manager_id} value={manager.manager_id}>{manager.display_name} · {manager.community_count} Communities</option>)}</select></label>{activityForm.managerId && <label>Exact Telegram Community<select value={activityForm.assetId} onChange={(e) => setActivityForm({ ...activityForm, assetId: e.target.value })}><option value="">Select exact Community</option>{selectedCreateAssets.map((asset) => <option key={asset.id} value={asset.id}>{asset.name}{asset.handle ? ` (@${asset.handle.replace(/^@/, '')})` : ''} · {Number(asset.audience_size || 0).toLocaleString()} audience · {human(asset.verification_status)}</option>)}</select><small>This exact Community, not only its manager, will own the campaign evidence.</small></label>}</> : null}</div><div className="ops-form-actions"><button type="button" className="ops-button ghost" onClick={() => setShowActivity(false)}>Cancel</button><button className="ops-button primary" disabled={!activityForm.title.trim() || !activityForm.destinationUrl.trim()}>Add activity</button></div></form></div>}
      ...
      {showOutcome && writable(project) && <div className="ops-modal-backdrop" onMouseDown={(e) => { if (e.currentTarget === e.target) setShowOutcome(false); }}><form className="ops-modal" onSubmit={recordOutcome}><div className="ops-modal-head"><div><span className="ops-kicker">OUTCOME LEDGER</span><h2>Record an outcome</h2></div><button type="button" onClick={() => setShowOutcome(false)}>×</button></div><label>Tracking link<select value={outcomeForm.trackedLinkId} onChange={(e) => setOutcomeForm({ ...outcomeForm, trackedLinkId: e.target.value })} required><option value="">Select tracking link</option>{links.filter((link) => link.status !== 'archived').map((link) => <option key={link.id} value={link.id}>{link.activity_title || link.code} · {link.code}</option>)}</select></label><div className="ops-field-grid two"><label>Outcome type<input value={outcomeForm.eventType} onChange={(e) => setOutcomeForm({ ...outcomeForm, eventType: e.target.value })} placeholder="registration, sale, activation" required /></label><label>Attributed value, optional<input type="number" min="0" step="0.01" value={outcomeForm.valueUsd} onChange={(e) => setOutcomeForm({ ...outcomeForm, valueUsd: e.target.value })} placeholder="250" /></label></div><label>External outcome ID<input value={outcomeForm.eventKey} onChange={(e) => setOutcomeForm({ ...outcomeForm, eventKey: e.target.value })} placeholder="CRM, order or signup ID" required /><small>Use a unique ID from your own system. Linkary prevents duplicates.</small></label><div className="ops-form-actions"><button type="button" className="ops-button ghost" onClick={() => setShowOutcome(false)}>Cancel</button><button className="ops-button primary" disabled={!outcomeForm.trackedLinkId || !outcomeForm.eventKey.trim()}>Record outcome</button></div></form></div>}
    </div>
  </ProductWorkspace>;
}
