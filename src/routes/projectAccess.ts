...
export async function listProjectMembers(request: Request, env: Env, organizationId: string): Promise<Response> {
  ...
  const members = await db.all<{ user_id: string; role: MemberRole; billing_manager: number; created_at: string; display_name: string; email: string | null }>(
    `SELECT m.user_id, m.role, m.billing_manager, m.created_at, u.display_name, u.email
...
export async function updateProjectMember(request: Request, env: Env, organizationId: string, userId: string): Promise<Response> {
  ...
  const timestamp = now();
  await db.batch([
    db.statement(`UPDATE organization_memberships SET role = ?, updated_at = ? WHERE user_id = ? AND organization_id = ?`, [body.role, timestamp, userId, organizationId]),
...
export async function removeProjectMember(request: Request, env: Env, organizationId: string, userId: string): Promise<Response> {
  ...
  const timestamp = now();
  await db.batch([
    db.statement(`UPDATE organization_memberships SET status = 'removed', billing_manager = 0, updated_at = ? WHERE user_id = ? AND organization_id = ?`, [timestamp, userId, organizationId]),
