...
export async function renderInviteLanding(request: Request, env: Env, code: string): Promise<Response> {
  ...
  return html(`<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><link rel="icon" type="image/png" href="/assets/brand/linkary-icon-black.png?v=20260909-v2"><link rel="shortcut icon" type="image/png" href="/assets/brand/linkary-icon-black.png?v=20260909-v2"><link rel="apple-touch-icon" href="/assets/brand/linkary-icon-black.png?v=20260909-v2"><title>You're invited to Linkary</title><style>:root{font-family:Inter,ui-sans-serif,system-ui;color:#171717;background:#f7f7f5}*{box-sizing:border-box}body{margin:0;min-height:100vh;display:grid;place-items:center;padding:24px;background:radial-gradient(circle at 50% 0,#fff3ea 0,transparent 38%),#f7f7f5}.card{width:min(520px,100%);background:#fff;border:1px solid #e7e5e4;border-radius:24px;padding:32px;box-shadow:0 24px 64px rgba(0,0,0,.08)}.mark{width:44px;height:44px;display:grid;place-items:center}.mark img{display:block;width:100%;height:100%;object-fit:contain}.eyebrow{margin-top:28px;color:#f26419;font-weight:800;font-size:12px;text-transform:uppercase;letter-spacing:.12em}h1{font-size:36px;line-height:1.08;margin:10px 0 12px}p{color:#525252;line-height:1.6;font-size:16px}.types{display:flex;gap:8px;flex-wrap:wrap;margin:20px 0}.pill{padding:8px 11px;background:#fafafa;border:1px solid #e7e5e4;border-radius:999px;font-size:13px}.cta{display:block;text-align:center;background:#171717;color:#fff;padding:14px 18px;border-radius:13px;text-decoration:none;font-weight:750;margin-top:24px}.note{font-size:12px;color:#8a8a8a;margin-top:14px;text-align:center}@media(max-width:520px){body{padding:14px}.card{padding:24px;border-radius:18px}h1{font-size:31px}}</style></head><body><main class="card"><div class="mark"><img src="/assets/brand/linkary-icon-black.png?v=20260909-v2" alt="Linkary"></div><div class="eyebrow">Private network invitation</div><h1>You're invited to Linkary.</h1><p>Continue to the Linkary app and sign in with Email, Google, or X. Your invitation is only redeemed after secure authentication succeeds.</p><div class="types">${types.map((t)=>`<span class="pill">${escapeHtml(t === 'project' ? 'Company / Project' : 'Creator')}</span>`).join('')}</div><a class="cta" href="${escapeHtml(continueUrl)}">Continue to Linkary</a><div class="note">Single-use invitation. Linkary referral attribution is measured directly by Linkary.</div></main></body></html>`, { headers });
}
