...
export async function renderInviteLanding(request: Request, env: Env, code: string): Promise<Response> {
  ...
  const continueUrl = `${getLinkaryUrls(request, env).api}/api/auth/x/start?invite=${encodeURIComponent(code)}&return_to=%2Fonboarding`; const types = JSON.parse(invite.allowed_account_types_json) as string[];
