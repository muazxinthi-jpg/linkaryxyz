...
export async function renderInviteLanding(request: Request, env: Env, code: string): Promise<Response> {
  ...
  const params = new URLSearchParams({ invite: code, return_to: '/onboarding' });
  const continueUrl = `${getLinkaryUrls(request, env).app}/login?${params.toString()}`;
  const types = JSON.parse(invite.allowed_account_types_json) as string[];
