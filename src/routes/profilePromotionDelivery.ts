...
.page.linkary-promotion-page{overflow:visible!important}
.linkary-sponsored-header{--linkary-avatar-overlap:clamp(96px,9.4vw,120px);--linkary-cta-avatar-gap:clamp(20px,2.35vw,24px);position:relative;z-index:5;width:min(1230px,calc(100vw - 48px));margin:22px 50% 0;transform:translateX(-50%);border-radius:28px;overflow:visible;isolation:isolate}
.linkary-sponsored-banner{display:block;width:100%;height:clamp(330px,34vw,420px);border-radius:28px;overflow:hidden;background:#f2f2f2;border:1px solid rgba(255,85,0,.18);box-shadow:0 18px 44px rgba(38,24,17,.14)}
.linkary-sponsored-banner img{width:100%;height:100%;display:block;object-fit:cover}
.linkary-sponsored-curve{position:absolute;z-index:6;left:0;right:0;bottom:0;height:clamp(100px,12vw,150px);overflow:hidden;pointer-events:none}
.linkary-sponsored-curve svg{display:block;width:100%;height:100%}
...
.linkary-sponsored-header + script + .linkary-promotion-hero,.linkary-sponsored-header + .linkary-promotion-hero{margin-top:calc(0px - var(--linkary-avatar-overlap))!important;padding-top:0!important;position:relative!important;z-index:9!important}
.linkary-promotion-hero .avatar{width:clamp(180px,18.75vw,240px)!important;height:clamp(180px,18.75vw,240px)!important;position:relative!important;z-index:10!important;border:5px solid #fff!important;border-radius:50%!important;box-shadow:0 14px 38px rgba(42,28,20,.16)!important}
@media(max-width:640px){.linkary-sponsored-header{--linkary-avatar-overlap:72px;--linkary-cta-avatar-gap:20px;width:calc(100vw - 24px);margin-top:14px;border-radius:20px}.linkary-sponsored-banner{height:280px;border-radius:20px}.linkary-sponsored-curve{height:80px}.linkary-sponsored-cta{height:40px;min-width:112px;padding:0 18px}.linkary-featured-project{max-width:62%;font-size:10px}.linkary-promotion-hero .avatar{width:clamp(132px,40vw,168px)!important;height:clamp(132px,40vw,168px)!important;border-width:4px!important}.linkary-sponsored-header + script + .linkary-promotion-hero,.linkary-sponsored-header + .linkary-promotion-hero{margin-top:-72px!important}}
</style>`;
...
export async function enhancePublicProfileWithPromotion(response: Response, request: Request, env: Env, username: string): Promise<Response> {
  ...
  if (creative) {
    ...
    const markup = `<section class="linkary-sponsored-header" data-linkary-sponsored="${escapeHtml(creative.auction_id)}" aria-label="Sponsored promotion"><a class="linkary-sponsored-banner" href="${escapeHtml(bannerHref)}" target="_blank" rel="sponsored noopener noreferrer"><img src="${escapeHtml(creative.banner_url)}" alt="Sponsored banner"></a><span class="linkary-sponsored-curve" aria-hidden="true"><svg viewBox="0 0 1000 94" preserveAspectRatio="none"><path d="M0 0Q500 94 1000 0V94H0Z" fill="#fff" stroke="rgba(255,85,0,.18)" stroke-width="1" vector-effect="non-scaling-stroke"/></svg></span><a class="linkary-sponsored-cta" href="${escapeHtml(ctaHref)}" target="_blank" rel="sponsored noopener noreferrer">${escapeHtml(ctaLabel(creative.cta_type))}</a><span class="linkary-sponsored-label">Sponsored</span></section><script>(function(){try{fetch(${JSON.stringify(impressionEndpoint)},{method:'POST',keepalive:true,credentials:'omit'}).catch(function(){});}catch(e){}})();</script>`;
