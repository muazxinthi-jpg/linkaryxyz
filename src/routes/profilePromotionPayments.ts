...
export async function verifyPromotionPayment(request: Request, env: Env, auctionId: string): Promise<Response> {
  ...
  const auction = await db.first<{ live_duration_hours: number; live_duration_days: number | null }>(`SELECT s.live_duration_hours, s.live_duration_days FROM profile_promotion_auctions a JOIN profile_promotion_slots s ON s.id = a.slot_id WHERE a.id = ?`, [auctionId]);
  if (!auction) throw new HttpError(404, 'Auction not found', 'auction_not_found');
  const promotionEndsAt = new Date(Date.now() + (auction.live_duration_days ? auction.live_duration_days * 24 : auction.live_duration_hours) * 60 * 60 * 1000).toISOString();
  await db.batch([
