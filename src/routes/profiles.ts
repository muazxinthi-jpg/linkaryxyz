...
async function renderPublicProfileV2(request: Request, env: Env, username: string): Promise<Response> {
  ...
  const images = blocks.filter((block) => block.block_type === 'featured_image' && block.url).slice(0, 4);
  ...
  const rain = Array.from({ length: 32 }, (_, index) => {
    ...
    const stream = Array.from({ length: 30 }, (_item, row) => {
      const glyph = row % 7 === 0 ? word[(row + index) % word.length] : glyphs[(index * 19 + row * 11) % glyphs.length];
      const opacity = Math.max(.08, 1 - row / 25).toFixed(2);
      return `<b style="--o:${opacity}">${escapeHtml(glyph)}</b>`;
    }).join('');
  ...
  const mediaCard = (block: ProfileBlockRow, kind: string) => {
    ...
    const visual = media?.kind === 'video' ? `<video src="${escapeHtml(media.src)}" muted playsinline loop autoplay preload="metadata"></video>` : media?.kind === 'image' ? `<img src="${escapeHtml(media.src)}" alt="" loading="lazy" referrerpolicy="no-referrer">` : '<b>✦</b>';
    return `<a class="feature" href="${escapeHtml(go(block))}">${visual}<span><small>${kind}</small><strong>${escapeHtml(block.title || 'Featured work')}</strong><em>Explore ↗</em></span></a>`;
  };
  const gallery = (items: ProfileBlockRow[], label: string, className = '') => items.length ? `<section class="gallery ${className}"><h2>${label}</h2><div>${items.map((block) => { const config = safeJson(block.config_json) as { mediaUrl?: string }; const image = safePublicImageUrl(config.mediaUrl || '') || ''; return `<a href="${escapeHtml(go(block))}">${image ? `<img src="${escapeHtml(image)}" alt="" loading="lazy" referrerpolicy="no-referrer">` : '<b>✦</b>'}<span>${escapeHtml(block.title || label)}</span></a>`; }).join('')}</div></section>` : '';
  ...
  let css = `:root{font-family:Inter,ui-sans-serif,system-ui,sans-serif;background:#100d0b;color:#fff}*{box-sizing:border-box}body{margin:0;min-height:100vh;background:radial-gradient(700px 500px at 50% -10%,#ff4b1f55,transparent 70%),radial-gradient(560px 430px at 0 90%,#ff4b1f33,transparent 72%),#100d0b}.rain{position:fixed;inset:0;z-index:3;pointer-events:none;overflow:hidden;mix-blend-mode:screen}.rain i{position:absolute;left:calc(var(--x)*1%);top:-36vh;display:block;width:15px;word-break:break-all;color:#ff7048;font:700 11px/1.34 ui-monospace,monospace;text-shadow:0 0 12px #ff4b1f;opacity:.62;animation:fall var(--d) linear var(--l) infinite}@keyframes fall{to{transform:translateY(185vh)}}.shell{position:relative;z-index:2;width:min(720px,calc(100% - 28px));margin:30px auto;padding:18px 18px 44px;border:1px solid #ff704866;border-radius:28px;background:linear-gradient(180deg,#1d100ddd,#100d0bee 45%);box-shadow:0 30px 100px #000b}.top{display:flex;justify-content:space-between;align-items:center}.brand{color:#fff;text-decoration:none;font-weight:850}.brand:before{content:'≡';display:inline-grid;place-items:center;width:25px;height:25px;margin-right:7px;border-radius:7px;background:#ff4b1f;color:#100d0b}.share{border:1px solid #ffffff33;border-radius:999px;padding:8px 13px;background:#ffffff0d;color:#fff;font:700 12px inherit}.hero{text-align:center;padding:50px 10px 30px}.avatar{width:88px;height:88px;margin:auto;display:grid;place-items:center;overflow:hidden;border:2px solid #ff8d6b;border-radius:28px;background:#ff4b1f;font-size:30px;font-weight:900}.avatar img{width:100%;height:100%;object-fit:cover}.eyebrow{margin-top:15px;color:#ff9f85;font:700 10px ui-monospace,monospace;letter-spacing:.15em}.hero h1{margin:9px 0 4px;font-size:clamp(38px,8vw,62px);letter-spacing:-.07em;line-height:.95}.handle{color:#ffbba9;font-size:13px}.bio{max-width:560px;margin:17px auto 0;color:#fff3ee;line-height:1.55}.socials{display:flex;justify-content:center;gap:10px;margin-top:22px}.socials a{width:42px;height:42px;display:grid;place-items:center;border:1px solid #ffb19a55;border-radius:14px;background:#ff4b1f22;color:#fff;text-decoration:none;font-weight:850}.features{display:grid;gap:12px}.feature{position:relative;min-height:230px;overflow:hidden;border:1px solid #ff7048;border-radius:22px;background:linear-gradient(135deg,#38170f,#ff4b1f);color:#fff;text-decoration:none}.feature img,.feature video{position:absolute;width:100%;height:100%;object-fit:cover;opacity:.8}.feature:after{content:'';position:absolute;inset:0;background:linear-gradient(0deg,#100d0bf2,transparent 70%)}.feature span{position:absolute;z-index:1;left:20px;bottom:18px;display:grid;gap:6px}.feature small,.gallery h2,.links h2{color:#ffb29e;font:700 10px ui-monospace,monospace;letter-spacing:.14em}.feature strong{font-size:28px;letter-spacing:-.05em}.feature em{font-size:12px;font-style:normal}.gallery,.links{margin-top:28px}.gallery h2,.links h2{text-align:center;margin:0 0 11px}.gallery>div{display:grid;grid-template-columns:repeat(2,1fr);gap:10px}.gallery.nfts>div{grid-template-columns:repeat(4,1fr)}.gallery a{position:relative;min-height:165px;overflow:hidden;border:1px solid #ffb09a66;border-radius:18px;background:#26130f;color:#fff;text-decoration:none}.gallery img{width:100%;height:100%;object-fit:cover}.gallery span{position:absolute;left:10px;right:10px;bottom:9px;font-size:12px;font-weight:800;text-shadow:0 1px 5px #000}.links{display:grid;gap:9px}.links h2{margin-bottom:2px}.links a{display:grid;grid-template-columns:34px 1fr 20px;align-items:center;gap:10px;padding:13px 14px;border-radius:16px;background:#fff;color:#1b100d;text-decoration:none;font-weight:800}.links a span{color:#ff4b1f}.links a b{color:#a84b35}@media(max-width:620px){.shell{width:100%;margin:0;border-radius:0;min-height:100vh}.hero{padding-top:42px}.gallery.nfts>div{grid-template-columns:repeat(2,1fr)}.rain i:nth-child(2n){display:none}}@media(prefers-reduced-motion:reduce){.rain i{animation:none;top:calc((var(--x)%20)*5vh)}}`;
  css += `.rain i{word-break:normal!important}.rain i b{display:block;opacity:var(--o);font:inherit;font-weight:inherit}.signals{display:flex;flex-wrap:wrap;justify-content:center;gap:7px;margin:18px auto 0;max-width:570px}.signals:before{content:'PUBLIC SIGNALS';width:100%;color:#ffb29e;font:700 9px/1 ui-monospace,monospace;letter-spacing:.15em}.signals span{border:1px solid #ff704855;border-radius:999px;background:#ff4b1f16;padding:6px 9px;color:#ffcfbf;font:700 9px/1 ui-monospace,monospace;letter-spacing:.07em}`;
  const signalHtml = `<div class="signals">${[...identitySignals, ...networkSignals].filter(Boolean).slice(0, 5).map((signal) => `<span>${escapeHtml(signal)}</span>`).join('')}</div>`;
  const body = `<div class="rain" aria-hidden="true">${rain}</div><main class="shell"><header class="top"><a class="brand" href="${escapeHtml(urls.publicSite)}">Linkary</a><button class="share" data-share>Share</button></header><section class="hero"><div class="avatar">${avatar}</div><div class="eyebrow">${profile.profile_type === 'project' ? 'VERIFIED PROJECT' : 'CREATOR IDENTITY'}</div><h1>${escapeHtml(profile.display_name)}</h1><div class="handle">@${escapeHtml(profile.username)}</div>${profile.bio ? `<p class="bio">${escapeHtml(profile.bio)}</p>` : ''}<nav class="socials">${socialHtml}</nav>${signalHtml}</section>${featured.length ? `<section class="features">${featured.map((block) => mediaCard(block, block.block_type.replace('_', ' ').toUpperCase())).join('')}</section>` : ''}${gallery(images, 'FEATURED IMAGES')}${gallery(products, 'PRODUCT FEATURES')}${gallery(nfts, 'NFT COLLECTION', 'nfts')}${linkHtml}${proofHtml(proof)}${opportunitiesHtml(opportunities, urls.app)}</main><script>(function(){var b=document.querySelector('[data-share]');if(!b)return;b.addEventListener('click',function(){navigator.clipboard&&navigator.clipboard.writeText('${escapeHtml(canonical)}');b.textContent='Copied';setTimeout(function(){b.textContent='Share'},1200)})})();</script>`;
