...
async function renderPublicProfileV2(request: Request, env: Env, username: string): Promise<Response> {
  ...
  const previewImage = new URL('/assets/brand/linkary-banner.jpeg', canonical).toString();
  const blockUrl = (block: ProfileBlockRow) => `${canonical}/go/${encodeURIComponent(block.id)}`;
  const socialTypes = new Set(['social_link', 'telegram', 'youtube', 'tiktok', 'instagram', 'facebook', 'reddit', 'linkedin']);
  ...
  const socials = blocks.filter((block) => socialTypes.has(block.block_type) && block.url);
  const links = blocks.filter((block) => !socialTypes.has(block.block_type) && !['featured_video', 'featured_article', 'featured_image', 'team_member'].includes(block.block_type) && block.url);
  const icon = (block: ProfileBlockRow) => { const key = `${block.block_type} ${block.title || ''}`.toLowerCase(); if (key.includes('telegram')) return '✈'; if (key.includes('youtube')) return '▶'; if (key.includes('linkedin')) return 'in'; if (key.includes('instagram')) return '◎'; if (key.includes('tiktok')) return '♪'; if (key.includes('discord')) return '◉'; if (key.includes('x') || key.includes('twitter')) return '𝕏'; return '↗'; };
  ...
  const socialHtml = socials.map((block) => `<a class="social" href="${escapeHtml(blockUrl(block))}" aria-label="${escapeHtml(block.title || 'Social link')}">${icon(block)}</a>`).join('');
  const featureHtml = features.map((block, index) => { const thumbnail = block.block_type === 'featured_video' ? youtubeThumbnail(block.url) : null; return `<a class="feature ${index === 0 ? 'hero-feature' : ''}" href="${escapeHtml(blockUrl(block))}">${thumbnail ? `<img src="${escapeHtml(thumbnail)}" alt="" loading="lazy">` : '<span class="feature-art">◆</span>'}<span class="feature-shade"></span><span class="feature-copy"><small>${escapeHtml(block.block_type.replace('featured_', 'FEATURED ').toUpperCase())}</small><strong>${escapeHtml(block.title || 'Open featured work')}</strong><i>Explore ↗</i></span></a>`; }).join('');
  const linkHtml = links.map((block) => `<a class="link-card" href="${escapeHtml(blockUrl(block))}"><b>${icon(block)}</b><span>${escapeHtml(block.title || block.url || 'Open link')}</span><i>↗</i></a>`).join('');
  const teamHtml = teams.map((block) => `<a class="team-card" href="${escapeHtml(blockUrl(block))}"><b>${escapeHtml((block.title || '?').slice(0, 1).toUpperCase())}</b><span><strong>${escapeHtml(block.title || 'Team member')}</strong><small>${escapeHtml(String((safeJson(block.config_json) as { role?: string }).role || 'Team member'))}</small></span><i>↗</i></a>`).join('');
