...
async function renderPublicProfileV2(
  ...
  const defaultPreviewImage = new URL(
  ...
  const firstFeatureImage = features
    .map((block) => {
  ...
  const previewImage = firstFeatureImage || avatarUrl || defaultPreviewImage;
  ...
  let featureHtml = features
    .map((block, index) => {
      ...
      const media = resolved?.kind === "video"
        ...
        : resolved?.kind === "image"
          ? `<img src="${escapeHtml(resolved.src)}" alt="" loading="lazy" referrerpolicy="no-referrer" onerror="this.hidden=true;var f=this.nextElementSibling;if(f)f.hidden=false"><span class="feature-art" hidden>◆</span>${resolved.youtube ? '<span class="feature-play" aria-hidden="true">▶</span>' : ''}`
          : fallback;
  ...
  const teamHtml = teams
    .map((block) => {
      ...
      const image = teamAvatar
        ? `<img src="${escapeHtml(teamAvatar)}" alt="" loading="lazy" referrerpolicy="no-referrer">`
        : escapeHtml((block.title || "?").slice(0, 1).toUpperCase());
      ...
      const [projects, people] = await Promise.all([
        ...
        db.all<{ name: string }>(
          `SELECT DISTINCT u.display_name AS name
           ...
           JOIN organization_memberships colleague
             ON colleague.organization_id = mine.organization_id
             AND colleague.status = 'active'
           JOIN users u ON u.id = colleague.user_id
           WHERE mine.user_id = ? AND mine.status = 'active' AND colleague.user_id != ?
           ORDER BY u.display_name ASC LIMIT 6`,
          [profile.owner_user_id, profile.owner_user_id],
        ),
      ]);
      rainTokens = [
        profile.display_name,
  ...
  const matrixColumns = Array.from({ length: 24 }, (_, index) => {
    const token = rainTokens[index % Math.max(rainTokens.length, 1)] || "LINKARY";
    const next = rainTokens[(index + 3) % Math.max(rainTokens.length, 1)] || "IDENTITY";
