...
export async function renderPublicProfileCard(request: Request, env: Env, username: string): Promise<Response> {
  ...
  const linePoints = months.map((month, index) => `${70 + index * 360 / 11},${363 - Number(month.count) / max * 92}`).join(' ');
  const bars = destinations.length ? destinations.map((row, index) => `<text x="620" y="301" fill="#d8d5ce" font-size="15" font-family="Arial">${svgText(row.title || 'Other', 20)}</text><rect x="735" y="288" width="300" height="14" rx="7" fill="#262a27"/><rect x="735" y="288" width="${Math.max(8, 300 * Number(row.count) / Math.max(clicks, 1))}" height="14" rx="7" fill="#ff6500"/><text x="1055" y="301" fill="#f8f4ed" font-size="15" font-family="Arial">${Math.round(Number(row.count) / Math.max(clicks, 1) * 100)}%</text>`).join('') : '<text x="620" y="315" fill="#aaa9a2" font-size="15" font-family="Arial">Measured destinations will appear here</text>';
