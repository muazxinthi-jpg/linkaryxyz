...
async function renderPublicProfileV2(request: Request, env: Env, username: string): Promise<Response> {
  ...
  const gallery = (items: ProfileBlockRow[], label: string, className = '') => items.length ? `<section class="gallery ${className} count-${items.length}"><h2>${escapeHtml(sectionTitle(items, label))}</h2><div>${items.map((block) => { const config = safeJson(block.config_json) as { mediaUrl?: string }; const image = safePublicImageUrl(config.mediaUrl || '') || ''; return `<a href="${escapeHtml(go(block))}">${image ? `<img src="${escapeHtml(image)}" alt="" loading="lazy" referrerpolicy="no-referrer" onerror="this.hidden=true">` : '<b class="gallery-placeholder">✦</b>'}<span>${escapeHtml(block.title || label)}</span></a>`; }).join('')}</div></section>` : '';
