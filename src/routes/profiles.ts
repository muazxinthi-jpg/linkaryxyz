...
export async function profileAnalytics(request: Request, env: Env, profileId: string): Promise<Response> {
  ...
  if (hasSeriesFilters) {
    ...
    const linkParams: unknown[] = linkId === 'all'
      ? [profileId, `${fromDate}T00:00:00.000Z`, `${endDate}T23:59:59.999Z`]
      : [profileId, `${fromDate}T00:00:00.000Z`, `${endDate}T23:59:59.999Z`, linkId];
    ...
    filteredSeries = {
      range, interval, from: fromDate, to: endDate, linkId,
      profileViews: series.reduce((sum, point) => sum + point.profileViews, 0),
      linkClicks: series.reduce((sum, point) => sum + point.linkClicks, 0),
      series,
      links: blocks.filter((block) => block.url).map((block) => ({ id: block.id, label: clickDestinationLabel(block) })),
