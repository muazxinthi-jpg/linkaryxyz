...
import { exchangeCdpSession } from './auth/cdp';
...
const STATIC_OR_SYSTEM = new Set(['', 'index.html', 'styles.css', 'script.js', 'uilib.md', 'favicon.ico', 'assets', 'api', 'onboarding', 'admin', 'app', 'robots.txt', 'sitemap.xml', 'pricing', 'about', 'blog', 'privacy', 'terms', 'support', 'help', 'status', 'security', 'login']);
...
async function handleApi(request: Request, env: Env): Promise<Response> {
  ...
  if(path==='/api/health') return json({ok:true,service:'linkary',version:'cdp-auth-foundation',database:env.DB?'bound':'not-bound',auth:'coinbase_cdp',urls:getLinkaryUrls(request,env)});
  if(path==='/api/auth/me'){if(request.method!=='GET')return methodNotAllowed(['GET']);const auth=await getAuthContext(request,env);return json({authenticated:Boolean(auth),user:auth?{id:auth.user.id,displayName:auth.user.display_name,email:auth.user.email,superadmin:auth.isSuperadmin}:null});}
  if(path==='/api/auth/cdp/session'){if(request.method!=='POST')return methodNotAllowed(['POST']);return exchangeCdpSession(request,env);}
  // Temporary fallback while CDP is rolled out. New UI must use /api/auth/cdp/session.
